...
FIGURE8_SPEED_SCALE = 0.7  # update if you change it in the flight script
FIGURE8_XY_SCALE = 0.6
...
AUTONOMOUS_XY_SCALE = 0.7      # SCALE=0.7 in the script
...
TRAJECTORIES = {
    ...
    'autonomous': (autonomous_poly4d, AUTONOMOUS_SPEED_SCALE, AUTONOMOUS_XY_SCALE, AUTONOMOUS_IS_LOOP),
}
...
def load_csv(path):
    rows = {c: [] for c in ['time_s', 'x', 'y', 'z', 'vx', 'vy', 'vz',
...
def plot_analysis(data, segs, traj_type, speed_scale, xy_scale, loop, csv_path):
    ...
    plan = np.array([eval_poly4d(segs, t, speed_scale, xy_scale, loop)
                     for t in times])

    # Planned attitude from differential flatness
    planned_att = np.array([
        compute_planned_attitude(segs, t, speed_scale, xy_scale, loop)
        for t in times
    ])
    ...
    # Errors
    err_3d = np.sqrt((data['x'] - px)**2 + (data['y'] - py)**2 + (data['z'] - pz)**2)
    ...
    plan_full = np.array([eval_poly4d(segs, t, speed_scale, xy_scale, loop)
                          for t in t_plan_full])
    ...
    for axis, actual, planned, col in [
        ...
        ax.plot(times, planned, color=col, lw=1.0, ls='--', alpha=0.6, label=f'{axis} plan')
    ...
    ax.axhline(rmse_3d * 100, color='k', lw=0.8, ls='--', alpha=0.5,
    ...
    for att, col, key, pdata in [
        ...
        ax.plot(times, pdata, color=col, lw=1.0, ls='--', alpha=0.65,
                label=f'{key} planned')
    ax.set_xlabel('time [s]')
...
def main():
    ...
    parser.add_argument('--csv', default=None,
                        help='Path to flight CSV (default: latest in logs/)')
    parser.add_argument('--type', choices=['circle', 'figure8', 'autonomous'], default=None,
    ...
    if traj_type is None:
        ...
        if 'circle' in base:
            ...
            print('Cannot infer trajectory type from filename. Use --type circle|figure8|autonomous')
            sys.exit(1)
    ...
    print(f'Loaded {len(data["time_s"])} rows, t=[{data["time_s"][0]:.2f}, {data["time_s"][-1]:.2f}] s')
