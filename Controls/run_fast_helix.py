#!/usr/bin/env python3
"""
Fast Helix Flight — 2× speed ascending + descending spiral via full-state setpoints
====================================================================================
Same analytic helix as run_helix.py but LAP_TIME = 5.25 s (half of 10.5 s).
OMEGA_C doubles → centripetal acceleration quadruples → tilt ≈ 2.5° vs 0.6° at 1×.

No QP re-solve needed — the helix is analytic (closed-form flatness), not polynomial.
Halving LAP_TIME directly doubles OMEGA_C and VZ_UP.
...
Usage:
    ~/.pyenv/versions/flying_robots/bin/python run_fast_helix.py
...
LAP_TIME     = 5.25    # s — one full circle  (2× faster than run_helix.py)
...
def fly_helix(scf):
    ...
    logger = FlightLogger(cf, name="fast_helix")
    logger.start()
...
if __name__ == "__main__":
    ...
    print(f"FAST HELIX: r={HELIX_RADIUS}m  dz={HELIX_DZ}m  lap={LAP_TIME}s  2 laps  (2× faster)")
