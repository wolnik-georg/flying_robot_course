...
cd flying_drone_stack && cargo test
```
...
cd flying_drone_stack && cargo run --release --bin main -- --maneuver hover

# Circle r=0.25 m, ω=0.30 rad/s, 50 s (~2.4 laps)
cd flying_drone_stack && cargo run --release --bin main -- --maneuver circle

# Figure-8 a=0.25 m b=0.15 m, ω=0.5 rad/s, 40 s
cd flying_drone_stack && cargo run --release --bin main -- --maneuver figure8

# Autonomous exploration (frontier-based, SCAN→NAVIGATE→LAND)
cd flying_drone_stack && cargo run --release --bin main -- --maneuver explore

# Any maneuver with AI Deck camera (VO + loop closure + SLAM enabled)
cd flying_drone_stack && cargo run --release --bin main -- --maneuver circle --ai-deck
cd flying_drone_stack && cargo run --release --bin main -- --maneuver explore --ai-deck
```
...
cd flying_drone_stack && cargo run --release --bin mekf_eval -- runs/<file>.csv

# Analyse VO, loop closure, and pose graph from a flight CSV
cd flying_drone_stack && cargo run --release --bin slam_eval -- runs/<file>.csv

# Build a 3D occupancy map from one or more flight CSVs → PLY file
cd flying_drone_stack && cargo run --release --bin build_map -- runs/<file>.csv

# Test AI Deck camera connection (bench test, no flight)
cd flying_drone_stack && cargo run --release --bin ai_deck_test -- --frames 50 --save-all
```
...
~/.pyenv/versions/flying_robots/bin/python flying_drone_stack/scripts/plot_flight_diagnostic.py          # auto-picks latest CSV

# Shadow controller evaluation: roll/pitch cmd vs actual
~/.pyenv/versions/flying_robots/bin/python flying_drone_stack/scripts/plot_shadow_eval.py flying_drone_stack/runs/<file>.csv

# View a saved map in MeshLab or CloudCompare
meshlab flying_drone_stack/results/data/map.ply
```

## Assignment Binaries (simulation only)

```bash
cd flying_drone_stack && cargo run --release --bin assignment1   # Integrator comparison (Euler/RK4/Exp)
cd flying_drone_stack && cargo run --release --bin assignment2   # SE(3) geometric control, figure-8 tracking
cd flying_drone_stack && cargo run --release --bin assignment3 -- --csv "../State Estimation/logging_ekf/logging/fr00.csv"
cd flying_drone_stack && cargo run --release --bin assignment4   # Minimum-snap spline + differential flatness
cd flying_drone_stack && cargo run --release --bin assignment5 -- circle   # Safe-space sim (hover/circle/fig8) + MEKF
```
...
4. **Run**: `cd flying_drone_stack && cargo run --release --bin main -- --maneuver <name>` (with `--ai-deck` if using camera).
5. **Evaluate**: `~/.pyenv/versions/flying_robots/bin/python flying_drone_stack/scripts/plot_flight_diagnostic.py`
