//! Helix Test — ascending then descending helix via full-state setpoints at 25 Hz
//!
...
//!
//! Usage: cargo run --release --bin helix_test
...
const LAP_TIME:      f32  = 10.5;  // s — one full circle
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("=== Helix Test ===");
    println!("  r={HELIX_RADIUS:.2}m  dz={HELIX_DZ:.2}m  lap={LAP_TIME:.1}s  2 laps");
    ...
    // ── Helix (2 laps) ────────────────────────────────────────────────────
    println!("=== HELIX: 2 laps × {LAP_TIME:.1} s  (ascending + descending) ===");
