//! Fast Figure-8 Test — spline figure-8 at 2× speed (12 s total) via full-state setpoints
//!
//! Same as spline_figure8_test.rs but with 2 s per segment (12 s total) instead of 4 s.
//! At 2× speed: tilt ≈ 12°, thrust ≈ 0.30 N vs ≈ 0.27 N at 1× — blade flapping and
//! battery sag create unmodelled torques that INDI rejects automatically.
//!
//! Requires OOT firmware: cd firmware_app && make cload
//!
//! Usage: cargo run --release --bin fast_figure8_test
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Controller: {ctrl_name} (type {controller}) | Mode: Fast Figure-8  2× speed");
    ...
    // ── Fast Spline Figure-8 (2× speed: 2 s per segment = 12 s total) ────
    println!("=== Fast Spline Figure-8  2× speed  (2 s/seg × 6 = 12 s) ===");
    ...
    let durations = vec![2.0_f32; 6];  // 6 segments × 2 s = 12 s (2× faster than spline_figure8_test)
    ...
    let total_time = Duration::from_secs(24); // two figure-8 loops
    ...
    println!("Fast Figure-8 Test finished.");
    Ok(())
}
