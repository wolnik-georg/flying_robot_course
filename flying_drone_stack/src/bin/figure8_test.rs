//! Figure-8 Test — starts exactly like your working circle_test, then does a slow figure-8
//!
//! Usage: cargo run --release --bin figure8_test -- --controller 6
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Controller: {ctrl_name} (type {controller}) | Mode: Figure-8");
    ...
    // ── Ramp + Hover — identical to working version ──────────────────────────
    ...
    // ── Figure-8 Trajectory ──────────────────────────────────────────────────
    println!("=== Figure-8 Trajectory (slow) ===");
    let a = 0.25_f32;   // horizontal size of the 8
    let b = 0.18_f32;   // vertical size of the 8
    let figure_start = Instant::now();
    let total_time = Duration::from_secs(45);   // one slow figure-8

    while Instant::now() < figure_start + total_time {
        let t = (Instant::now() - figure_start).as_secs_f32();
        let angle = 2.0 * std::f32::consts::PI * t / 22.5;   // one figure-8 every ~22.5 seconds

        let cx = ox + a * angle.sin();
        let cy = oy + b * angle.sin() * (2.0 * angle.cos());

        cf.commander.setpoint_position(cx, cy, 0.50, oyaw_rad).await?;
        sleep(Duration::from_millis(70)).await;   // reasonably smooth
    }
