//! Flip Test — 360° pitch flip via full-state setpoints at 25 Hz
//!
//! Plans a smooth 0→2π angle trajectory using SplineTrajectory::plan
//! (2 segments, z-dimension = angle θ).  Evaluates θ(t) and ω(t) directly
//! from traj.eval(); no differential flatness needed (quat computed from θ).
//!
//! Requires OOT firmware: cd firmware_app && make cload
//!
//! Usage: cargo run --release --bin flip_test
...
const FLIP_HEIGHT:   f32  = 1.2;   // m — extra clearance during inversion
const T_FLIP:        f32  = 0.7;   // s — total flip duration

// ── Helpers (shared pattern with spline_circle_test.rs) ────────────────────
...
// ── Flip trajectory planning ────────────────────────────────────────────────

/// Plan smooth 0→2π angle trajectory via QP (2 segments, z-axis = θ).
/// Interior derivatives (at θ=π) are free and QP-optimised for min-snap.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Plan trajectory before connecting (so any panic happens early)
    println!("Planning flip trajectory...");
    let traj = plan_flip();
    ...
    // Upright quaternion at drone's yaw orientation
    ...
    // ── Flip ─────────────────────────────────────────────────────────────
    println!("=== FLIP: 0→360° in {T_FLIP:.2} s  (peak ω={:.0}°/s) ===",
    ...
    while Instant::now() < flip_end_t {
        ...
        let theta = flat.pos.z;   // angle [rad]
        let omega = flat.vel.z;   // angular rate [rad/s]

        // Pure pitch rotation: body y-axis.  q = [cos(θ/2), 0, sin(θ/2), 0] in [w,x,y,z]
        let qw = (theta / 2.0).cos();
        let qy = (theta / 2.0).sin();

        cf.commander.setpoint_full_state(
            ox, oy, FLIP_HEIGHT,
            0.0, 0.0, 0.0,        // velocity: hold position
            0.0, 0.0, 0.0,        // acceleration: zero
            qw, 0.0, -qy, 0.0,   // quaternion [w, x, y, z]  (-qy = backward flip)
            0.0, -omega, 0.0,     // angular rates [rad/s]: wx, wy, wz
        ).await?;
    ...
    // ── Recovery hold (upright, 1 s) ─────────────────────────────────────
    println!("Flip complete — recovery hold 1 s...");
