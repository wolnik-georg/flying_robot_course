...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for step in 0..25usize {                       // increased steps for smoother ramp
        let height = 0.02 + step as f32 * (0.48 / 24.0);
        cf.commander.setpoint_position(ox, oy, height, oyaw_rad).await?;
        sleep(Duration::from_millis(120)).await;   // slightly slower ramp
    ...
    println!("Test completed at target height 0.50 m.");
