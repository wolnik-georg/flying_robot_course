...
//!
//! Outputs: results/assignment4/data/
//!   assignment4_planned.csv     — sampled planned trajectory + flatness outputs
//!   assignment4_openloop.csv    — open-loop (flatness feedforward only)
//!   assignment4_closedloop.csv  — closed-loop (geometric controller + flatness)
...
fn main() {
    ...
    // ── Aircraft parameters ──────────────────────────────────────────────────
    // 1 kHz physics timestep for high-resolution planning and integration.
    let dt = 0.001_f32;
    ...
    let hover_yaw = 0.0_f32;
    let hover_duration = 1.5_f32;
    ...
    waypoints.push(Waypoint { pos: hover_pos, yaw: hover_yaw }); // pre-hover
    waypoints.extend(main_waypoints.iter().cloned());
    waypoints.push(Waypoint { pos: hover_pos, yaw: hover_yaw }); // post-hover

    let mut durations = Vec::new();
    durations.push(hover_duration);
    let n_main_seg = main_waypoints.len() - 1;
    durations.extend(vec![1.0_f32; n_main_seg]);
    durations.push(hover_duration);
    ...
    let mut ol_records: Vec<Record> = Vec::with_capacity(n_steps);
    ...
    // Geometric controller tracks the spline with flatness-based feedforward.
    ...
    let mut cl_records: Vec<Record> = Vec::with_capacity(n_steps);
    ...
    std::fs::create_dir_all("results/assignment4/data")
        .expect("Failed to create results/assignment4/data");

    let planned_path = "results/assignment4/data/assignment4_planned.csv";
    write_planned_csv(planned_path, &traj, dt, n_steps, &params);

    let ol_path = "results/assignment4/data/assignment4_openloop.csv";
    write_record_csv(ol_path, &ol_records);

    let cl_path = "results/assignment4/data/assignment4_closedloop.csv";
    write_record_csv(cl_path, &cl_records);

    println!("  {}", planned_path);
    println!("  {}", ol_path);
    println!("  {}", cl_path);
    ...
    for r in records {
        writeln!(f,
            "{:.6},{:.6},{:.6},{:.6},{:.6},{:.6},{:.6},\
             ...
             {:.6},{:.6},{:.6},{:.6}",
            r.t,
            r.ref_x, r.ref_y, r.ref_z, r.sim_x, r.sim_y, r.sim_z,
            r.ref_vx, r.ref_vy, r.ref_vz, r.sim_vx, r.sim_vy, r.sim_vz,
            r.ref_thrust, r.ref_tx, r.ref_ty, r.ref_tz, r.ref_yaw,
...
fn write_planned_csv(path: &str, traj: &SplineTrajectory, dt: f32, n_steps: usize,
...
fn print_errors(records: &[Record]) {
    ...
    let rms = |v: Vec<f32>| (v.iter().map(|x| x * x).sum::<f32>() / n).sqrt();
    ...
    let e3d: Vec<f32> = ex.iter().zip(&ey).zip(&ez)
        .map(|((x, y), z)| (x*x + y*y + z*z).sqrt()).collect();
    println!("  RMS 3D = {:.1} mm  (x={:.1} mm  y={:.1} mm  z={:.1} mm)",
        rms(e3d)*1000.0, rms(ex)*1000.0, rms(ey)*1000.0, rms(ez)*1000.0);
}
