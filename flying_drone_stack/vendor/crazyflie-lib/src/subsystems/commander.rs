...
impl Commander {
    ...
    /// Sends a full-state setpoint: position + velocity + acceleration + attitude (quaternion) + angular velocity.
    /// This is CRTP generic setpoint type 6 (fullState).
    /// pos in m, vel in m/s, acc in m/s², omega in rad/s.
    /// Packet layout: 1 + 6 + 6 + 6 + 4 + 6 = 29 bytes (fits in CRTP max 31).
    pub async fn setpoint_full_state(
        ...
        omegax: f32, omegay: f32, omegaz: f32,
    ) -> Result<()> {
        let quat = compress_quaternion(qw, qx, qy, qz);
        let mut payload = Vec::with_capacity(29);
        payload.push(TYPE_FULL_STATE);
        // Position in mm as i16
        payload.extend_from_slice(&((px * 1000.0) as i16).to_le_bytes());
        payload.extend_from_slice(&((py * 1000.0) as i16).to_le_bytes());
        payload.extend_from_slice(&((pz * 1000.0) as i16).to_le_bytes());
        // Velocity in mm/s as i16
        payload.extend_from_slice(&((vx * 1000.0) as i16).to_le_bytes());
        payload.extend_from_slice(&((vy * 1000.0) as i16).to_le_bytes());
        payload.extend_from_slice(&((vz * 1000.0) as i16).to_le_bytes());
        // Acceleration in mm/s² as i16
        payload.extend_from_slice(&((ax * 1000.0) as i16).to_le_bytes());
        payload.extend_from_slice(&((ay * 1000.0) as i16).to_le_bytes());
        payload.extend_from_slice(&((az * 1000.0) as i16).to_le_bytes());
        // Compressed quaternion as u32
        payload.extend_from_slice(&quat.to_le_bytes());
        // Angular velocity in mrad/s as i16
        payload.extend_from_slice(&((omegax * 1000.0) as i16).to_le_bytes());
        payload.extend_from_slice(&((omegay * 1000.0) as i16).to_le_bytes());
        payload.extend_from_slice(&((omegaz * 1000.0) as i16).to_le_bytes());
        let pk = Packet::new(GENERIC_SETPOINT_PORT, GENERIC_SETPOINT_CHANNEL, payload);
        self.uplink.send_async(pk).await.map_err(|_| Error::Disconnected)?;
        Ok(())
    }
...
/// Smallest-3 quaternion compression matching the Crazyflie firmware decoder.
/// The largest component is omitted; the remaining three are encoded as 10-bit values.
fn compress_quaternion(w: f32, x: f32, y: f32, z: f32) -> u32 {
    let abs = [w.abs(), x.abs(), y.abs(), z.abs()];
    let largest = abs.iter().enumerate()
        .max_by(|a, b| a.1.partial_cmp(b.1).unwrap()).unwrap().0;
    let (idx, a, b, c) = match largest {
        0 => { let s = if w < 0.0 { -1.0 } else { 1.0 }; (0u32, x * s, y * s, z * s) }
        1 => { let s = if x < 0.0 { -1.0 } else { 1.0 }; (1u32, w * s, y * s, z * s) }
        2 => { let s = if y < 0.0 { -1.0 } else { 1.0 }; (2u32, w * s, x * s, z * s) }
        _ => { let s = if z < 0.0 { -1.0 } else { 1.0 }; (3u32, w * s, x * s, y * s) }
    };
    let scale = core::f32::consts::SQRT_2 / 2.0;
    let enc = |v: f32| -> u32 { ((v / scale + 1.0) * 511.5) as u32 & 0x3FF };
    (idx << 30) | (enc(a) << 20) | (enc(b) << 10) | enc(c)
}
