#!/usr/bin/env python3
"""Runs one side (reference or ours) of test_naindi_reference.py's comparison in an
isolated subprocess, so the two same-named `cffirmware` extension modules never collide
in one Python process. Prints "thrust,tx,ty,tz" to stdout. Invoked by
test_naindi_reference.py, not meant to be run directly."""
import json
import math
import sys
...
def run_ours(fw_ours, case, gains):
    fw_ours.controllerOutOfTree2Init()
    ...
    fw_ours.naindi_test_set_j(*gains["J"])
    fw_ours.naindi_test_set_arm(gains["arm"], gains["t2t"])
    ...
    for tick in range(0, 40):
        fw_ours.controllerOutOfTree2(control, sp, sensors, st, tick)
    return control.thrustSi, (control.torqueX, control.torqueY, control.torqueZ)
