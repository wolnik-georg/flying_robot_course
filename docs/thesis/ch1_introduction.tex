...
\begin{description}
  ...
  \item[RQ3] How large is the transfer penalty when a summed per-neighbour residual model is
    ...
    not, and that is already established~\cite{shi2020neuralswarm,gielis2023aggregate}. The
    question applies to the predictive model and to the predictive term of the hybrid. It does not
    apply to a purely reactive law.

  \item[RQ4] For each strategy, what sensing, onboard compute and training data does it require,
    and how do those costs sit next to the tracking and rejection it delivers? In particular, is the
    extra complexity of the hybrid justified in the multi-robot case?
\end{description}
...
RQ4 compares cost against performance for every strategy, and then asks whether the hybrid's extra
bill is worth it when the residual comes from neighbours.
...
\begin{enumerate}
  \item \textbf{A controlled comparison.} To the best of our knowledge the first evaluation of
    ...
    methodological content is in the word \emph{controlled}, and it applies uniformly to every
    strategy: each is its own controller, and none is held to a stronger or weaker standard than
    another. Two of them, pure INDI (Strategy~1) and the hybrid (Strategy~4), happen to share an
    attitude law and differ only in which residual term is active, which is a useful point of
    comparison but not a privileged category the others are excluded from. The design provides: a
    frozen scenario library so every strategy meets identical excitation; gains fixed before data
    collection so a difference cannot be a tuning artefact; one residual definition logged
    identically under every controller, including those that do not use it; and null conditions in
    which no interaction is expected, so that a positive result is separable from noise.
...
\textbf{Chapter~\ref{ch:control}} describes Strategies~0--4 (the uncompensated baseline, pure INDI,
geometric+NN, FBL+NN and the hybrid), and the point at which each
