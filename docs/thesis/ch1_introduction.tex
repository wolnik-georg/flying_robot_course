...
\begin{enumerate}
  \item \textbf{A controlled comparison.} To the best of our knowledge the first evaluation of
    ...
    methodological content is in the word \emph{controlled}, and it applies uniformly to all seven
    strategies: each is its own controller, and none is held to a stronger or weaker standard than
...
consume a force residual use that number\ifadvancedtier; Strategies~5 and~7 do not form
it\fi. It gives
...
\textbf{Chapter~\ref{ch:control}} describes \ifadvancedtier Strategies~0--7 (eight controllers: an
uncompensated baseline plus seven families, with residual reinforcement learning split into two
variants by inner loop)\else Strategies~0--4 (the uncompensated baseline, pure INDI,
geometric+NN, FBL+NN and the hybrid)\fi, and the point at which each
