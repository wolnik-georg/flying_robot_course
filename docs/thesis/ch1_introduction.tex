...
When several quadrotors fly close together, the downwash of the upper vehicle can destabilise the
one below. The usual remedy is to stay far enough apart that the interaction can be ignored. For a
Crazyflie~2.0 (\SI{9}{\centi\metre} rotor-to-rotor) that conservative planning gap has been taken
as about \SI{60}{\centi\metre}; Shi et al.~\cite{shi2020neuralswarm} cite that figure from earlier
planning work as one such example.

That margin is expensive. It sets the density at which a team can operate, and therefore the size
of the space a given number of vehicles requires. Applications that motivate multirotor teams in the first place --- inspection in confined spaces,
cooperative transport, aerial docking --- are precisely those in which the space is not available. In some of them, close approach is the task
...
beyond about two and a half motor-to-motor distances below the
vehicle~\cite{bauersfeld2024airflow}. Formation flight happens closer than that. In that near
field those authors report no closed-form model.
...
The two sources are complementary. Measurement is late but not tied to a training set. Prediction
can act earlier but only where it was trained. Combining them is the obvious next construction. It
has been built, on a single vehicle. There the authors describe the extra gain over the reactive
method as small without a payload, and as leaving performance similar to the reactive method when a
payload is attached~\cite{cobobriesewitz2026lindi}.
...
which is where the predictive component has the most to contribute; multi-robot evaluation of such
hybrids is not in the scope of this thesis. And the summation that
...
\begin{description}
  \item[RQ1] Under identical hardware, trajectories, formations and separations, how do reactive
    and predictive interaction-force compensation strategies compare to the uncompensated baseline
    in trajectory-tracking accuracy and residual-force rejection?
  ...
  \item[RQ3] How large is the transfer penalty when a summed per-neighbour residual model is
    trained only on two-vehicle data and then used on three-vehicle formations, and is that penalty
    small enough that the summed architecture remains the right engineering choice? This measures
    the size of a known approximation. It is not a test of whether superposition holds: it does
    not, and that is already established~\cite{shi2020neuralswarm,gielis2023aggregate}. The question
    applies to the predictive strategies (Strategy~2 and, if implemented, Strategy~3). It does not
    apply to a purely reactive law.

  \item[RQ4] For each compared strategy, what sensing, onboard compute and training data does it
    require, and how do those costs sit next to the tracking and rejection it delivers?
\end{description}

RQ1 is the primary question and the one the experimental design serves. RQ2 carries the most
engineering value, because it converts a ranking into a rule about when to use which. RQ3 asks for
the magnitude of the transfer penalty on a deployed model, not whether superposition holds.
RQ4 compares cost against performance for every compared strategy.
...
This thesis sets out to establish the following. This list is the intended contribution, not a
report of measured outcomes. The experiments that would support it are specified in
Chapter~\ref{ch:setup} and, once flown, belong in the results chapters.

\begin{enumerate}
  \item \textbf{A controlled comparison.} To the best of our knowledge, the first evaluation of
    reactive and predictive compensation on the same airframe, the same trajectories and the same
    formations, with one definition of the residual logged under every controller, including those
    that do not use it, and with null flights that establish the measurement floor.
    ``Controlled'' means the experiment is held fixed, not that every law is given the same gains.
    Geometric tracking, INDI and feedback linearisation are different controllers; each is tuned for
    itself. Gains are then frozen before the comparison flights so a later difference is not
    mid-campaign retuning.
...
Chapter~\ref{ch:setup} and offered for reuse.
...
and gains frozen after each strategy is tuned are for.
...
\textbf{Chapter~\ref{ch:model}} defines the residual this stack measures and logs. Strategies that
consume a force residual use that number. It gives
...
\textbf{Chapter~\ref{ch:control}} describes Strategies~0--3 (the uncompensated baseline, pure INDI,
geometric+NN and FBL+NN), and the point at which each
...
measurement chain and the protocol, and states the threats to validity the design is built against, including those it does not eliminate.

\textbf{The two- and multi-robot results chapters} report the two- and three-vehicle results.
\textbf{The discussion chapter} interprets them against the research questions and the stated
assumptions, and \textbf{the concluding chapter} concludes.
