...
Chapter~\ref{ch:control} described Strategies~1--6 plus 7a and 7b (eight controllers; seven families, with residual
reinforcement learning split by inner loop), and argued that four of them differ only in
...
zero} rather than failing loudly. Strategies 2 and 5 additionally cannot run at all. This is the
...
Section~\ref{sec:ctrl-s1}. This is not a convenience: a compensating controller partly cancels the
...
The comparison is tightest among Strategies 1, 2, 3 and 5, which share the entire position and
attitude loop and differ only in which residual term is active; a shared gain set is meaningful for
them in a way it is not across structurally different controllers. Strategies 4 and 6 are compared
