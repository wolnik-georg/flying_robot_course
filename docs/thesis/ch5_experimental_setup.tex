...
Chapter~\ref{ch:control} described Strategies~0--4 (the uncompensated baseline, pure INDI,
geometric+NN, FBL+NN and the hybrid), each treated as its own controller. This chapter describes the platform,
the scenarios, the measurement chain and the protocol, and
...
with per-vehicle role alignment replaced radio-only windows for thesis metrics. Indicative
Strategy~0 vs~1 comparisons exist; the systematic comparison (C.4) still requires repeated
flights per controller and scenario. Residual-model training data collection (C.1) runs under
...
controls C1--C3 are static; A2 and A3 are quasi-static; A4 and A5
...
uncompensated baseline. And the network prediction is recorded \emph{whether it is enabled or not},
...
Each strategy is compared under identical trajectories, formations, hardware and frozen gains, not
identical control structure — this holds uniformly across all of them, not just a subset.
Pure INDI (1) and the hybrid (4) additionally share the entire position and attitude loop, differing
only in which residual term is active, which the discussion chapter can point to when interpreting
their results, without treating the other strategies' comparisons as weaker on that account.
...
commanded trajectory. Alongside these, the attitude tracking error, reported as the RMSE and peak
...
that visible. The logging configuration described in Section~\ref{sec:setup-recorded} records
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}p{0.28\linewidth}p{0.34\linewidth}p{0.30\linewidth}@{}}
    ...
    Radio vs onboard clock & uSD logs at \SI{500}{\hertz}; analysis uses merged uSD and firmware \texttt{ctrltarget.*} where available & Residual-time alignment across vehicles depends on merge quality; \texttt{merge\_usd\_logs.py} aborts above \SI{15}{\centi\metre} RMS role check \\
    Mocap identity / marker swap & Pre-takeoff EKF-vs-mocap gate; role verification from merge & Residual risk if a session bypasses the gate \\
...
\todo{Continue revising to past tense for each scenario and controller row as C.4 repeats land.
Record any deviation from the frozen gain block or scenario parameters in this chapter.}
