...
Chapter~\ref{ch:control} described \ifadvancedtier Strategies~0--7 (eight controllers: an
uncompensated baseline plus seven families, with residual reinforcement learning split into two
variants by inner loop)\else Strategies~0--4 (the uncompensated baseline, pure INDI, geometric+NN,
FBL+NN and the hybrid)\fi, each treated as its own controller. This chapter describes the platform,
...
identical control structure — this holds uniformly across all seven, not just a subset of them.
