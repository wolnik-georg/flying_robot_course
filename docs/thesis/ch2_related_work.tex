...
\ifadvancedtier\else
The survey covers the field as a whole, including families this thesis does not itself implement.
Where a family falls outside the strategies evaluated here --- learning the corrective action
directly (Section~\ref{sec:rw-rl}) and embedding the residual in a predictive-control horizon
(Section~\ref{sec:rw-inject}) --- it is described briefly rather than in depth, and the omission is
one of scope, not of relevance: those families are part of the landscape the gap argument of
Section~\ref{sec:rw-eval} rests on, and are developed in full in the companion Advanced-tier draft.
...
constraint of a nonlinear MPC; Li et al.~\cite{li2023ndpnmpc} inject a predicted downwash-force
sequence into the optimiser's prediction model; and Hsieh et al.~\cite{hsieh2025onlineadaptation}
add an $\mathcal{L}_1$ online estimate to the same family. All three run the optimisation
off-board or on a companion computer rather than on the vehicle. This injection point is not among
the strategies evaluated here --- its onboard feasibility on a Crazyflie-class platform is itself
open --- and it is developed in full in the companion Advanced-tier draft; it is retained in this
survey because its evaluation conditions form part of the argument in
...
This family is not among the strategies evaluated in this draft, for that second reason as much as
for scope: the evaluation built in Chapter~\ref{ch:setup} rests on the residual being a measurable
quantity, and a method that never forms one is assessed on tracking performance alone. It is
developed in full, as two strategies distinguished by inner loop, in the companion Advanced-tier
draft.
\fi
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}llll@{}}
    ...
    Hsieh et al.~\cite{hsieh2026flatness}       & 2                  & \makecell[l]{\SI{0.3}{\metre} and \SI{0.4}{\metre}\\ vert.\textsuperscript{\ddag}} & RMSE and max.\ $z$ error \\
    Chee et al.~\cite{chee2024knodedwmpc}       & 2, Crazyflie~2.1   & \makecell[l]{commanded \SIrange{0.2}{0.4}{\metre};\\ tight case \(\approx\)\SI{0.08}{\metre}} & \makecell[l]{RMSE / max.\ $z$ vs.\\ nominal MPC; off-board} \\
    Li et al.~\cite{li2023ndpnmpc}               & 2, custom          & neighbour-relative & \makecell[l]{NMPC with predicted\\ force sequence; Jetson + mocap} \\
    Hsieh et al.~\cite{hsieh2025onlineadaptation} & 3                & V-stack and I-stack; pairwise \SI{0.2}{\metre} & tracking vs.\ MPC family \\
    Shankar et al.~\cite{shankar2023docking}    & 2, custom (\(\approx\)\SI{0.27}{\metre}) & dock \(\approx\)\SI{0.36}{\metre} & \makecell[l]{terminal vert.\ error /\\ dock success; onboard Pi, mocap} \\
    Cobo-Briesewitz et al.~\cite{cobobriesewitz2026lindi} & 1 (+ payload) & n/a                 & position tracking error \\
    Zhang et al.~\cite{zhang2024proxfly}        & 2, heterogeneous   & \makecell[l]{\SI{0.50}{\metre} vert.\ hover/circle;\\ dock to \SI{0.10}{\metre}, release at \SI{0.05}{\metre}} & position and attitude error \\
    \bottomrule
  \end{tabular}
