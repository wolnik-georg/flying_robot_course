...
The chapter treats each strategy as its own controller. Two of them, pure INDI and the hybrid,
happen to build their attitude law the same way and differ only in which residual term is active
in the shared desired-acceleration vector \eqref{eq:ctrl-ades}; that is a genuine similarity worth
stating where it clarifies the comparison, but it is not treated here as a formal category that
some strategies belong to and others do not. What makes the comparison fair across all seven is not
a claimed shared implementation, but the experimental conditions held constant for every one of
them — the same airframe, trajectories, formations, residual definition and frozen gains, stated
precisely in Section~\ref{sec:ctrl-fairness}.
...
Strategies~1, 2, 3 and~5 (the geometric baseline, pure INDI, geometric+NN and the hybrid) each use
a geometric position loop and an attitude law realising the corresponding thrust direction; INDI
and the hybrid replace the attitude law's inner term with an incremental one. This is stated once
here, in its own section, purely to avoid repeating the same equation four times — it is a
description of what each of those four controllers does, not a claim that they form a privileged
group for the purposes of the comparison. Strategies~4, 6 and~7 use different published structures
entirely — feedback linearisation, receding-horizon optimisation and residual reinforcement
learning — described in their own sections instead.
...
\cite{zhang2024proxfly}; Strategy~7b sits on the geometric position and attitude loop used by
...
across strategies wherever the architecture permits. This holds uniformly for all seven
strategies — none is exempted or held to a looser standard. Where a controller's own structure
happens to make a comparison unusually direct (pure INDI and the hybrid differ only in which
residual term of \eqref{eq:ctrl-ades} is active, since the hybrid's attitude law is INDI's own),
that is noted in the relevant strategy section as a point of interest, not treated as a stronger
class of evidence than the rest of the comparison.
...
controller, ``identical conditions'' means identical trajectories, formations, hardware and gains —
not identical control structure. A difference in outcome could in principle be attributable to a
...
not necessarily favourable to all of them equally: gains chosen to suit one controller's operating
point may disadvantage another that could tolerate a different one.
...
suit. This thesis freezes shared gains because the tuning budget available here is one lab
session, not an automated search, and a shared operating point is the only one that can be applied
consistently within that budget. The discussion chapter reports where a frozen gain appears to have
