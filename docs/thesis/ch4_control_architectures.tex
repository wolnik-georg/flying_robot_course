...
% Draft-scope flag: \ifadvancedtier true = full 7-strategy draft (Advanced tier included);
% false = Minimum-tier draft (Strategies 0-4 only; 5-7 omitted). Set by the driver file
% (thesis_draft_advanced.tex / thesis_draft_minimum.tex) before this file is \input; defaults to
% true (full content) if undefined, so the original thesis_draft.tex and ch4_standalone.tex keep
...
Strategies~0--7 (eight controllers: an uncompensated baseline plus the seven families of
Section~\ref{ch:control}, with residual reinforcement learning split into two variants by inner
loop). Table~\ref{tab:ctrl-summary} lists them, and the rest of the chapter takes them in order.
...
\ifadvancedtier\else
\emph{This is the Minimum-tier draft: Strategies 5--7 (the reinforcement-learning-based
strategies) are out of scope here and are covered in the companion Advanced-tier draft. Nothing
...
Strategies~0, 1, 2, 4 and~7 (the geometric baseline, pure INDI, geometric+NN, the hybrid, and
residual RL on the geometric baseline) each use a geometric position loop and an attitude law
realising the corresponding thrust direction; INDI and the hybrid replace the attitude law's inner
term with an incremental one, and Strategy~7 instead adds a learned corrective action after the
position loop rather than a term inside it. This is stated once here, in its own section, purely
to avoid repeating the same equation five times — it is a description of what each of those five
controllers does, not a claim that they form a privileged group for the purposes of the comparison.
Strategies~3, 5 and~6 use different published structures entirely — feedback linearisation, a
cascaded residual-reinforcement-learning baseline, and receding-horizon optimisation — described in
...
\else
Four Minimum-tier strategies (1--4: pure INDI, geometric+NN, FBL+NN and the hybrid), plus the
uncompensated baseline (Strategy 0) flown as the reference condition under all of them, each its
own controller, distinguished by what they know about the interaction force,
when they know it, and where that knowledge enters. Two of them, pure INDI and the hybrid, happen to
share an attitude law and differ only in which residual term is active — a useful point of
comparison, not a privileged evidentiary category. All four are compared under the same
experimental conditions: identical trajectories, formations, hardware and frozen gains, not
identical control structure. The Advanced tier (Strategies 5--7, the reinforcement-learning-based
strategies) is covered in the companion Advanced-tier draft.
\fi

At the time of writing, \ifadvancedtier two strategies fly, one is implemented and unflown, one
exists as separable components, and three are not implemented\else two strategies fly (the
baseline and pure INDI), one is implemented and unflown (geometric+NN), one exists as separable
components (the hybrid), and one is not implemented (FBL)\fi. Chapter~\ref{ch:setup} describes the experimental
