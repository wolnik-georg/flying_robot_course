...
chapter describes the seven control strategies compared in this thesis, and in each case the point
...
differ by. All of them track the same trajectories, on the same airframe, through the same attitude
allocation. What separates them is which terms are present in one vector --- the desired
acceleration of the position loop --- and where that vector's information comes from. Presenting
them this way is not a simplification for exposition; it is the structural property that makes the
comparison fair, and Section~\ref{sec:ctrl-fairness} states precisely what it does and does not
guarantee.

Each strategy is described in two parts, kept deliberately separate: what is implemented here, and
the published method it follows. Our implementations are representatives of their families, not
reproductions of specific papers, and the deviations are stated rather than glossed.

% -------------------------------------------------------------------------------------
\section{The common substrate}
\label{sec:ctrl-common}

Every strategy is built on a geometric tracking controller on $SE(3)$. The position loop forms a
desired acceleration and the attitude loop realises the corresponding thrust direction.
...
information is present is what distinguishes Strategies 0, 1, 2 and 4 from one another}, and
nothing else about the position loop changes between them: Strategy~0 has neither term,
Strategy~1 the measured one, and Strategy~2 the predicted one.

Strategy~4, which has both, does not simply enable the two terms together. It uses the predicted
residual and then corrects with the \emph{difference} between measurement and prediction, for the
reason given in Section~\ref{sec:ctrl-s4}; \eqref{eq:ctrl-hybrid} states that form. Written as
...
\eqref{eq:ctrl-tau-geo} with an incremental law, described in Section~\ref{sec:ctrl-indi}.
...
no stability result proved there transfers to it unexamined.
...
\paragraph{Role.} Not one of the seven, but flown as a condition in every scenario. Without it,
``how much did compensation help'' has no denominator, and the interaction cannot be separated from
the model error discussed in Section~\ref{sec:model-measure-fail}. It is also the controller under
...
configuration; the intermediate ones are diagnostic and are used in the two-robot results chapter
to attribute an effect to one loop or the other.
...
(Section~\ref{sec:model-measure-fail}). No training data.
...
The prediction is computed and logged \emph{whether or not it is enabled}. This is what makes the
...
% -------------------------------------------------------------------------------------
\section{Strategy 4: hybrid neural-augmented INDI}
\label{sec:ctrl-s4}
...
prediction the incremental term vanishes and the strategy reduces to Strategy~2, and with no
prediction it reduces exactly to Strategy~1.
...
separately on every flight (Section~\ref{sec:ctrl-s2}): model quality is assessed from
...
training data --- the union of Strategies~1 and~2, which is itself a result for the cost accounting
...
this platform is an open question, and one that is itself relevant: this is the only strategy in
the comparison whose computational cost may exclude it from the target hardware.
...
% -------------------------------------------------------------------------------------
\section{Strategies 5 and 7: residual reinforcement learning}
\label{sec:ctrl-rl}
...
module on top of a \emph{cascaded} controller --- not a geometric one --- producing corrections that
compensate the disturbance and thrust loss caused by downwash, using domain randomisation to reduce
the dependence on accurate system identification. Their residual and high-level controller run
off-board at \SI{50}{\hertz} above an onboard low-level loop at \SI{500}{\hertz}, on regular-sized
quadcopters. Strategy~5 follows this arrangement; Strategy~7 pairs the same stance with
the geometric baseline used elsewhere in this thesis, a combination which is ours rather than any
paper's.

\paragraph{Why they are scheduled last, stated plainly.} They answer a different question.
Strategies~1--4 and~6 all reason about an explicit force: each produces, or consumes, an estimate
...
Table~\ref{tab:ctrl-summary} summarises the strategies along the three axes that distinguish them.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}clllcc@{}}
    ...
    4 & Hybrid NA-INDI            & both       & $a_{\mathrm{des}}$, torque & yes & form fixed, not wired \\
    5 & Residual RL               & neither    & actuation                 & no  & deferred \\
    6 & Learning-based MPC        & predicted  & MPC prediction model      & no  & not implemented \\
    7 & Geometric + residual RL   & neither    & actuation                 & no  & deferred \\
    \bottomrule
  \end{tabular}
\end{table}
...
across strategies wherever the architecture permits. Strategies 1, 2 and 4 additionally share the
...
Strategies 3 and 6 do not share the substrate: a feedback-linearising law and a receding-horizon
...
suit. This thesis freezes shared gains because strategies 1, 2 and 4 share the entire control loop,
...
Finally, each implementation is a representative of its family and not a reproduction of the paper
it follows. Where a strategy underperforms, the honest conclusion concerns \emph{this
implementation} of that family on \emph{this} platform, and the discussion chapter is
careful to phrase it that way.
