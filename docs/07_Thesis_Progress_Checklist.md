...
| 4 | **Hybrid Neural-Augmented INDI** | Yes, alongside the INDI measurement | `controller=8` — **slot patch built, unverified** | ⬜ 2026-09-16: `ControllerTypeOot3`/`CONFIG_CONTROLLER_OOT3` slot-patched into `controller.h`/`controller.c`/`Kconfig`, `app-config`, and `bindings/cffirmware.i`; `make bindings_python` links clean, `controllerOutOfTree3Init/Test` pass, dispatch reachable, NN forward pass returns sane non-degenerate output on a zero-input smoke test. **Still NOT numerically verified against the reference's own compiled C — do not fly.** `controller=7` is NOT this strategy — its `use_nn` flag is dead code, so it runs as Strategy 1's alt implementation, not NA-INDI |
