...
| **Next action — lab** | **⚠️ A PURE VALIDATION SESSION — C.0 stage 1 is BLOCKED until both controllers fly clean again. See [`docs/lab_sessions/2026-09-09.md`](lab_sessions/2026-09-09.md).** On 2026-09-09 six hover flights crashed under both controllers. **Three separate root causes** were found and fixed, none re-flown: (1) geometric ran INDI's position gains (`kp_xy=64/kv_xy=5`, ζ=0.31 — now selects `40/8`); (2) the residual sign flip `.add`→`.sub` (now parked at frozen behind `indi_gains.res_sign=+1`); (3) the landing path streamed `cmdFullState` before `land()`, which cannot work (now the plain figure8 sequence). Stock Lee hovered clean at roll std 0.74°, proving the vehicle is healthy. An [independent audit](REVIEW_FINDINGS_2026-09-09.md) confirmed 6/7 claims, downgraded the INDI attribution to unproven, and found 3 more defects (all fixed). **Next session — TWO STAGES. Stage 1: pull the uSD cards (free evidence, lost if overwritten) → reflash → three geometric flights (hover/circle/figure8, unaffected by every new switch) → **ONE** INDI hover on the restored config only. `check_flight.py` after every single flight, stop on the first FAIL. Stage 2, only if stage 1 passes, one variable per flight: (a) `res_fc=80`+`res_clamp=10`; (b) `filt_prewarp=1`+`filt_dt_us=1000`+`fc_bw=206`, a deliberate no-op that makes `fc_bw` honest; (c) tune `fc_bw` down; (d) re-test the notch, which has never actually been tested since it sat at 13.8 Hz not 6.9; (e) **the shake-band test** — our attitude loop's ω_n is **7.80 Hz** and the documented shake is 6.3–7.9 Hz, so drop `kr` 2400→987→632 (ω_n 5.0→4.0 Hz, ζ held at 1.74) and see whether the shake follows. Converted to a common basis our attitude gains are **8.2× stiffer than NA-INDI's**; the whole historical `kr` ladder (1500–2800) sat inside the shake band. See [`22`](22_NA_INDI_Repo_Survey.md) §2h.** Only after that do gain retuning and multi-drone resume. The decisive A1 residual-sign flight is still not reached |
...
| **Blocking** | **Both tracks now block on the same thing: flight data.** Lab track: the six-flight controller re-validation, then C.0. Writing track: **no longer "nothing"** — Ch. 1–5 are frozen (2026-09-09) and Ch. 6–9 are results/discussion/conclusion, which cannot be written before the campaign exists. The two-track parallelism that held for most of the project has ended; the writing track is complete to the data boundary, not idle by choice |
