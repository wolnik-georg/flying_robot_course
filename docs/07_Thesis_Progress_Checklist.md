...
| **Next action — lab** | **C.0 stage 1, still awaiting the next hardware session — see [`docs/lab_sessions/2026-09-07.md`](lab_sessions/2026-09-07.md).** INDI's config reverted to byte-identical with the flight-proven state (confirm next session); geometric needs its own new tuning ladder (never independently tunable before this session). Between sessions, all 16 formation-library scenarios were re-validated in sim against a fixed runner (landing-collision bug, missing sim pose telemetry, and a cross-run state-carryover wiggle all found and fixed) — see [`sim_scenario_commands_2026-09-08.md`](lab_sessions/sim_scenario_commands_2026-09-08.md). None of this substitutes for hardware: the decisive A1 residual-sign flight still has not happened on real hardware. Order at the next session: confirm clean hover on both controllers → common trajectories → A1 2-robot (doubles as the sign check) → freeze gains → C.1 |
