...
| **Next action — lab** | **2026-09-12 session done — see [`docs/lab_sessions/2026-09-12.md`](lab_sessions/2026-09-12.md) for the full account.** H0 partition run: both isolated sub-loops (`ctrl_mode=2`, `ctrl_mode=1`) crashed on hover alone, worse than full INDI — inconclusive, the two loops are too coupled to isolate this way. Full-INDI sanity check confirmed unchanged. **First-ever 2-drone flight achieved end to end** after finding and fixing five real bugs in `formation_flight.py` (missing `pos_gains` selection, a mid-air controller-identity-switch risk, missing hover support, arming gated behind an unused flag, a hardcoded log path that crashed the flight before landing, and a landing collision for vertical/stacked formations — all fixed, see the session doc). A mixed-platform (brushless + standard CF2.1) geometric attempt then crashed both drones; root-caused NOT to the attitude gain itself (verified bit-identical to a proven-good 2026-07-02 value via git history) but to the rest of the shared firmware config having drifted brushless-only since July. **Desk-time follow-up same day: the notch crash was root-caused and fixed offline (a third filter chain missed the 2026-07-29 cold-start fix, `flying_robot_course` `bc7190c`, builds clean, not yet flown), and the mixed-platform crash story was corrected with a bigger second bug — `apply()`'s broadcast was silently wiping `cf_second`'s per-robot mass/kt override on every call, fixed generally (`crazyswarm2` `fdfc640`). See History (23).** **Architecture decision 2026-09-13: `cf_second` is now permanently pinned to stock Lee (`crazyswarm2` `bb88e73`), regardless of platform — only `cf231_active` is ever the controller under study (geometric/INDI/future NA-INDI/IL-NDI). This removes the whole cross-platform gain-sharing risk by construction (stock Lee never reads `indi_gains`/`pos_gains`) and means the brushless swap for `cf_second` is no longer needed for that reason — it can stay standard CF2.1 indefinitely.** **2026-09-13 session: the `notch_en=1` test was flown — it CRASHED, worse than the original 2026-09-11 crash, with zero log data saved. The `bc7190c` fix does NOT resolve the notch problem; `notch_en` reverted to 0 and must not be re-enabled without real offline analysis (never actually done, despite being flagged since 2026-09-11). Most of the session was lost to an unrelated hardware/tooling problem first: the drone was found running the wrong firmware platform (standard CF2.1 flashed onto the brushless airframe), which also left it stuck in bootloader mode partway through debugging; both resolved by an explicit `DRONE=bl` reflash. Full account: `docs/lab_sessions/2026-09-13.md`.** **Desk-time follow-up same day: the offline notch analysis (bode/phase response, `experiments/analysis/notch_filter_bode.py`) was done — CONCLUSIVE. The attitude loop's own natural frequency (`ωₙ = √kr = 7.80 Hz` at `kr=2400`) sits almost exactly inside the notch's rejection band (`f0=6.9`, `bw=3.0`, i.e. ~5.4–8.4 Hz), where phase swings ~175° in under 0.1 Hz. The notch isn't filtering an external resonance — the "shake" likely IS the loop's own dynamics, so notching it corrupts the loop's feedback signal with huge phase distortion exactly at its operating point. **This is a structural problem, not a timing bug — no re-tuning of `notch_f0`/`notch_bw` fixes it while `kr`/`kw` keep `ωₙ` at 7.8 Hz. The notch-filter approach is abandoned for this problem.** The more promising path already has real supporting data: the coordinated `kr`/`kw` retuning from 2026-09-11 (attempt 4, `483/76` + geometric `pos_gains`, peak oscillation down to 6.7°).** **Planning note: neither our own geometric nor INDI has actually worked in a 2-drone scenario yet — the only clean 2-drone flight remains stock Lee (`a_res=0`, no real data); the one attempt at our own geometric on both drones is what crashed on 2026-09-12.** **2026-09-14 plan** (`docs/lab_sessions/2026-09-14.md`): (1) geometric, 2 drones, A1 hover (`cf231_active`=our geometric, `cf_second`=stock Lee) — closes C.0 flight 5 (`docs/11`) and *is* the first real C.1 data point; (2) full INDI, 2 drones, A1 hover — deliberately ahead of C.0's strict required order (figure-8 under corrected INDI hasn't reached a clean pass) since A1 is pure hover and doesn't exercise the open trajectory-tracking shake, an informed decision not an oversight; (3) uSD logging + sync validation on both drones, exercised during (1)/(2); (4) if clean, move to A3. The `kr`/`kw` + `pos_gains` retuning line (the actual fix for the trajectory-tracking shake) continues separately and does not block this plan. Brushless swap for `cf_second` explicitly deferred to its own session. **2026-09-14 evening: first clean 2-drone A8 on our own controller** (`cf231_active` geometric, `cf_second` stock Lee). **2026-09-15: uSD logging validated END TO END — the first usable two-drone dataset exists** (A8 19:56, both drones, 1.9/1.8 cm RMS vs commanded, ~20-30 ms cross-drone sync, `a_res` live, `dz` 0.193-0.283 m, closest approach 0.198 m with `a_res_z` -3.36 m/s²). Getting there required root-causing a crash-every-flight bug we had introduced ourselves the same evening — `usec.reset` broadcast **mid-flight** destroys the high-level commander's time base; it must only ever be sent on the ground before takeoff (`crazyswarm2` `e07342f`). Isolated by flying **A1 pure hover**, which failed identically and ruled out trajectory/geometry/downwash/controller/gains at once. Also fixed: per-drone logs now record each vehicle's **actual** controller and gains instead of the shared `all:` block (`a7d5d0f`) — every A8 since 09-14 had recorded `ctrl_mode=3` for a drone that flew `ctrl_mode=0`. See History (36) and `docs/lab_sessions/2026-09-15.md`. **Next lab session: C.1 data collection can begin** — the logging chain is no longer the blocker. |
| **Next action — writing** | **Chapters 1–5 frozen (2026-09-09) after a Related-Work polish** (ch2's 60 cm sentence aligned with ch1's locked wording; Table 2.1 gained six rows for already-discussed works). Next writing is Chapters 6–9, after C.1 data — no further related-work expansion until then |
...
| | |
|---|---|
| **Flight volume** | x −1..1, y −2..2, z 0..1.70 — **tape-measured by the operator 2026-09-02**, set in `FLIGHT_SPACE` in `formations/safety.py`. With `--auto-center --rotate 90` all 19 configs fit at full default parameters |
...
|---|---|---|---|
| 1 | ~~Tape-measure the flight volume~~ | Lab measurement | ✅ **Done 2026-09-02** — `FLIGHT_SPACE` in `safety.py` is a real measurement, not a placeholder |
...
- [x] Multi-robot flight script `formation_flight.py` — **sim-validated 2026-09-03, no open items**: 24/24 pair-checks PASS across all 3 formations x {2,3} drones x {geo,indi}, quantitatively verified against `record_states` (not just crash-free), all cleanly (no timeouts). Physics cross-check matches the scenario library's own signature (INDI cancels vertical downwash, coplanar cases near-zero under both controllers). The ~35-46 deg trajectory-boundary tilt is confirmed genuine commanded dynamics (`figure8 @ kt=0.05`'s own rest-to-rest acceleration ramp), not a bug — flagged on `C0_FLIGHT_CARD.md` — see 2026-09-03 (4)-(6) History entries
...
- [x] 10b. ~~Confirm the flight volume with a tape measure~~ — **done 2026-09-02**: x −1..1, y −2..2, z 0..1.70, set in `FLIGHT_SPACE` in `formations/safety.py` (no longer the placeholder)
...
| 4 | Tape-measure the flight volume | ✅ **Done 2026-09-02** |
...
### Desk work while lab-blocked

Additive to C.0, not a substitute for it. Nothing here changes gains, the residual sign, or any
control law, and none of it counts as C.0 progress by itself.

- [x] 1 Analysis script — `experiments/analysis/run_analysis.py` + `metrics.py`, 2026-09-08
- [x] 2 `e_R` logging — `indi.e_r_x/y/z/norm` (uSD mandatory, radio optional), 2026-09-08.
...
**C.0 remains NOT started / not flown.**
...
- [ ] Hardware inventory and bench checks (Checklist A); ~~tape-measure the flight volume~~ done 2026-09-02
...
| 4 | **Hybrid Neural-Augmented INDI** | Yes, alongside the INDI measurement | `controller=8` — **numerically verified; UNSTABLE in closed-loop sim, one root cause fixed, one still open** | ✅ built, ⬜ unflown. 2026-09-16: `ControllerTypeOot3`/`CONFIG_CONTROLLER_OOT3` slot-patched in, numerically verified 6/6 vs the reference's own compiled C to ~1e-9. **Same day, first CS2 SIL closed-loop run (single-drone hover) found both this and `controller=7` diverge into a crash partway through the simplest possible scenario**, while `controller=6` sails through identically. Root-caused one real bug: both files hardcoded the reference's own stock-CF2.1 arm/thrust-to-torque literals in the RPM→torque INDI model instead of this project's real per-platform values (mass/J/kt were already correct) — an 8% arm-length low bias. Fixed, made airframe-aware like `lib.rs`, static verification re-confirmed 100% at ~1e-9. **Sim divergence reduced but NOT eliminated** — still crashes at nearly the same point. Suspected residual cause: the reference's own attitude gains (KR/Komega), tuned for their airframe, may not be stable against this project's real CF21BL inertia (~44% larger Jxx/Jyy). **Not investigated further — needs its own dedicated pass. Do not fly either controller.** `controller=7` is NOT this strategy — its `use_nn` flag is dead code, so it runs as Strategy 1's alt implementation, not NA-INDI |
...
| Date | Change |
|---|---|
| 2026-09-16 (40) | **First CS2 SIL closed-loop run of controller=7/8 finds a real, shared instability — one root cause fixed, one still open.** Wired `oot2`/`oot3` into the CS2 SIL simulator (`crazyflie_sil.py`/`crazyflie_server.py`, `crazyswarm2`) for the first time — both controllers had only ever been exercised via the static, single-tick numerical test vectors from (39), which by construction never simulate a real plant. On the simplest possible test (single drone, straight hover, `crazyflies_sim1.yaml` + `simple_flight.py --trajectory hover`), **both controller=7 and controller=8 climb cleanly then diverge into a growing attitude oscillation and crash within ~3-4s**, at almost the same instant for both — while `controller=6` run through the identical scenario/config shows zero disturbance. Localized with an ad-hoc diagnostic print (removed before commit) plotting roll/pitch through the failure window: a genuine growing oscillation, not a script or HLC artifact. **Root-caused one real bug, shared between both files**: `naindi.rs`/`naindi_hybrid.rs` hardcoded the reference's own stock-CF2.1 arm/thrust-to-torque literals (`ARM_REF`, `T2T_REF`) in the RPM→torque INDI model, unlike mass/J/kt which already correctly track this project's real per-platform airframe (`g_indi_mass`/`JXX,JYY,JZZ`/`g_indi_kt1-4`) — an 8% arm-length low bias on the real CF21BL brushless platform (0.0325m reference vs 0.0354m real). Made airframe-aware with the same `#[cfg(drone_bl)]` switch `lib.rs` uses, plus a test-only override hook (`naindi_test_set_arm`/`naindi_hybrid_test_set_arm`) so (39)'s numerical-verification tests can still pin the reference's own literals — re-ran both, still 100% at ~1e-9 (5/5, 6/6). One regression caught and fixed along the way: fixing the reference build's `usecTimestamp()` stub for this required replacing an earlier call-counting approach (from (39), tuned for `use_nn=1`'s 3-calls-per-tick pattern) with a tick-indexed clock, since the call-counting version silently broke `controller=7`'s own `use_nn=0` test (1 call per tick) into all-NaN — the new tick-indexed stub is correct for both regardless of internal call count. **Sim divergence reduced but NOT eliminated by the arm/t2t fix** — both controllers still crash at nearly the same point in the identical scenario. Suspected residual cause, not investigated: the reference's own attitude gains (`KR`, `Komega`), tuned for their own airframe, may not be stable against this project's real CF21BL inertia (`JXX`/`JYY` ~44% larger than the reference's own `J`). **Do not fly either controller pending that investigation.** Also fixed one unrelated pre-existing bug found while using this path: `simple_flight.py` unpacked 5 values from `_load_firmware_controller_config`'s 6-tuple return (broke on any use of the script, any controller). Committed `flying_robot_course` `1ff8287`, `crazyswarm2` `10b569d`. |
| 2026-09-16 (39) | **`controller=8` numerically verified against the reference's own compiled C — 6/6 test vectors match to ~1e-9.** Following on from (38): wrote `test_naindi_hybrid_reference.py`/`_naindi_hybrid_case_runner.py`, extending `test_naindi_reference.py`'s exact pattern (controller=7) to `ctrl.use_nn=7` so the real trained network in `nn.c`/`nn_utils.c` is actually exercised, not just linked unused. Two build additions to the scratch reference: a settable `motorsGetRatio()` stub (the NN's own input vector reads it directly) and, more substantively, a `naindi_hybrid.rs`-side test-only `naindi_hybrid_test_set_j()` inertia-override hook (mirroring, not sharing, `controller=7`'s own — the two sides' real per-platform `J` don't match). First run: 3/6 FAIL, all on torque, all correlated with nonzero gyro/RPM asymmetry, never on the two all-zero-gyro cases. **Diagnosed before touching anything**: a direct ctypes call to the reference's own exported `nn_forward()` against our `naindi_hybrid_test_nn_forward()` on an identical input matched bit-for-bit — ruling out the port and pointing at the surrounding harness. Root cause: with `use_nn` enabled, `controller_lee.c` calls `usecTimestamp()` **three times** per active tick (two of them unused profiling calls bracketing the NN forward pass, one real for the attitude-INDI `dt` measurement); the scratch stub's flat "+2000us every call" therefore advanced the reference's clock 3x faster than this port's single call per tick, inflating `(omega-omega_prev)/dt`. Fixed by making the stub advance only on the 3rd call of each per-tick cycle, so elapsed time between real measurements stays exactly 2000us regardless of the diagnostic calls in between — after which all 6 cases (including one deliberately set with `gyro != gyro_reg` to catch a wiring swap between the NN's plain-gyro input and the INDI path's `gyroNoLpf`) match to ~1e-9. Full account: `firmware_app/host/naindi_reference_build_notes.md`'s "NA-INDI hybrid" addendum. **Genuinely still open: never flown.** Numerical verification is a precondition for flying `controller=8`, not a substitute for it — it must still clear the same hardware-validation gate (`docs/11`) as every other untested controller change. |
| 2026-09-16 (38) | **`controller=8` firmware slot patch built and smoke-tested — still not numerically verified.** Following on from (37c): wired `ControllerTypeOot3`/`CONFIG_CONTROLLER_OOT3` into `controller.h` (enum + declarations), `controller.c` (dispatch row), `Kconfig` (help text says plainly "NOT YET NUMERICALLY VERIFIED... do not fly"), `app-config` (`CONFIG_CONTROLLER_OOT3=y` alongside the existing two OOT flags), and `bindings/cffirmware.i` (both the `%{ %}` and plain SWIG blocks — these two are not byte-identical, an initial single-replacement script asserted incorrectly and had to be split in two). Added a `motorsGetRatio()` host stub to `oot_host.c` (mirrors the existing `rpm_get_all`/`oot_set_rpm` injection pattern) since `motors.c` — a real STM32 driver — is not part of the SIL/host build. `make bindings_python` from a real `~/Desktop/crazyflie-firmware` checkout links clean; `controllerOutOfTree3Init/Test` pass; `controllerOutOfTree3`/`naindi_hybrid_test_nn_forward`/`oot_set_pwm_ratio` are all present and callable in the compiled `_cffirmware*.so` (confirmed via `dir(cf)`, not just `nm`). A ctypes-level smoke test of `naindi_hybrid_test_nn_forward` on an all-zero input returned finite, non-degenerate values (not NaN, not all-zero) — evidence the ported network evaluates, not evidence it matches the reference. Regenerated `naindi_controller_slot.patch` and `cffirmware_bindings.patch` to include these changes and updated `firmware_app/host/LOCAL_MODIFICATIONS.md`; committed to `main` (`1dc0314`). **Genuinely still open:** full numerical verification against the reference's own compiled C (the `test_naindi_reference.py` pattern, extended to `use_nn=1`) has not been attempted — that is the actual next step before controller=8 can be trusted for anything, let alone flown. |
| 2026-09-16 (37) | **Desk session: `dataset.py` unblocked, the strategy↔controller mapping corrected, `controller=8` started.** (a) `dataset.py` rewritten to match `model.NeuralSwarm2`'s real contract (ground-effect input wired, `model.build_mask` replaces the removed distance guards, `z_floor` defaults to 0.0 so `phi_G` isn't starved) and verified end-to-end against the real A8 19:56 uSD data — `cf_second` correctly excluded as ego/target (a_res always 0, no RPM source) while still providing the neighbour state behind `cf231_active`'s 7518 rows. A second real bug found and fixed in passing: `load_merged()` didn't know about the `# meta:t_zero=...` comment line `merge_usd_logs.py --meta` writes, so it read that as the header. `train.py` still needs its own separate rewrite (imports the removed `DeepSets`). (b) **Strategy↔controller mapping corrected**: `controller=7` is Strategy 1's (Pure INDI) *second implementation* (Cobo-Briesewitz's own INDI baseline, one of their paper's own four compared methods), **not** Strategy 4 — their `use_nn` flag is off in the port, so it runs their plain INDI. True NA-INDI (Strategy 4) needs a separate `controller=8`. Published as `docs/strategy_controller_map.html`, linked from `docs/01`. (c) **`controller=8` started**: checking their repo directly (not assuming from the earlier `naindi.rs` note) found a REAL trained network in `nn.c` — 19→24→24→24→6, genuine calibration ranges (`X_maxs[15..19]` run to ~65500, matching `motorsGetRatio()`'s 16-bit range) — solving a *different* problem than this thesis (their own single-vehicle payload residual, own-state-only input, no relative/neighbour state at all). Wrote an isolated Rust module (`naindi_hybrid.rs` + `naindi_hybrid_weights.rs`, own state/FFI, no sharing with `controller=7`) porting the real weights and the full force+torque `use_nn` wiring line-for-line from `controller_lee.c`/`nn_utils.c`. Compiles clean on host target, zero errors. **Not yet done and stated plainly in the module's own doc: no controller-slot patch (`controller.c` dispatch + Kconfig), and NOT numerically verified against the reference's own compiled C** — a standalone link for a quick sanity check hit missing firmware globals (`g_indi_mass`, `g_indi_kt1-4`) that only have real values inside the full `crazyflie-firmware` host build; stubbing them with guessed values would not have proven anything, so that was not attempted. Do not fly, do not cite as verified. |
| 2026-09-15 (36) | **uSD logging chain VALIDATED end to end — first usable two-drone dataset — after root-causing a crash-every-flight bug that we had introduced ourselves.** Full account: [`docs/lab_sessions/2026-09-15.md`](lab_sessions/2026-09-15.md). **(a) The crash bug.** After the uSD tooling work, every A8 crashed at a *fixed elapsed time* regardless of `passes` or scenario length. Two theories were investigated and are recorded as wrong: battery sag (a *symptom* — the violent correction draws the current) and an `apply()` controller-identity race (a **genuine** bug, fixed and kept in `crazyswarm2` `9d9c3de`, but not the cause — the re-flight crashed identically). The decisive test was flying **A1, a pure hover with no trajectory motion**, which failed the same way and thereby eliminated trajectory content, crossing geometry, downwash, `dz`, `passes`, controller and gains in one flight. Root cause: **`usec.reset` was being broadcast mid-flight**, and the high-level commander's entire time base is that same clock (`crtp_commander_high_level.c`: `float t = usecTimestamp() / 1e6;`) with `t_begin` stored from it at takeoff — so zeroing it left `piecewise_eval` computing `t - t_begin` ≈ *minus* several hundred seconds, evaluating a degree-7 polynomial far outside its domain and handing the controller a garbage setpoint within one tick. Introduced 2026-09-14 in `formation_flight.py`/`flight.py` and propagated to `run_formation.py` the same evening. Fixed (`crazyswarm2` `e07342f`) by broadcasting it **once, on the ground, before takeoff**, where the planner is IDLE; uSD logs keep their shared origin, which was the only thing that ever mattered. Post-fix flights returned to the pre-bug baseline exactly (max roll 34.08° vs 33.70°/33.96°). **(b) The chain works.** A8 19:56, both drones: 500 Hz, 2.3 ms worst gap, both matching their commanded trajectory unambiguously at **1.9 / 1.8 cm RMS**, cross-drone clock agreement **~20–30 ms**, `indi.a_res_*` live. Realised geometry from the merged data: `dz` 0.193–0.283 m vs commanded 0.25, closest approach 0.198 m, and `a_res_z` reaching −3.36 m/s² there — the downwash interaction A8 exists to measure, recorded correctly on both vehicles for the first time. Workflow and data: `flying_drone_stack/tools/README_usd_thesis_logging.md`, `experiments/logs/usd_raw/`. **(c) Firmware fact corrected:** `usddeck.c` creates a **new file on every `usd.logging` 0→1** and `f_close` runs only on 1→0 — so a **0-byte `thesisNN` is a session that started but never stopped cleanly** (crash/power loss/card pulled), and the counter is not a timeline and not a drone identity (cards move between vehicles). An earlier claim that sessions append into one file per power cycle was wrong and is fixed. **(d) Tooling defects found against real data and fixed:** `find_flight_window.py` searched lags from 0 regardless of the recording's own `t[0]` and required only 100 samples (0.2 s) of overlap — together producing a confident *"STRONG match, 173× better than runner-up"* fitted to 0.2 s of tail; now requires ≥80% scenario coverage and reports RMS in cm. `merge_usd_logs.py` gained `--meta`/`--roles` to align each drone against **its own commanded trajectory** (its z-cross-correlation is untrustworthy whenever altitudes are flat — it reported +262 ms at corr 0.39 where the truth was ~20 ms), aborts if a log's fit to its claimed role exceeds 15 cm RMS (which is what now *verifies* drone/role identity from data rather than from an operator-typed filename — validated by deliberately swapping roles: 79.5 cm, refused), and its name parsing no longer truncates every real drone name. `copy_usd_log.py` now picks the highest-numbered non-empty file rather than the largest (the largest was a 14.9 MB corrupted recording from hours earlier). **Known limitation, recorded not papered over:** correlation proves a file holds *a run* of a scenario, not *which* run — two identical back-to-back A8s matched both meta files at 1.7/1.8 cm, so ordering repeated runs relies on the `thesisNN` counter, an assumption about session order. **(e) Per-drone config now recorded correctly** (`crazyswarm2` `a7d5d0f`): the shared `all:` block used to be written verbatim into every drone's log, so every A8 since 09-14 recorded `ctrl_mode=3`/full INDI for `cf231_active`, which actually flew `ctrl_mode=0`/geometric (proof: `tau_x/y/z` exactly 0.0 across all 7637 uSD samples). Per-drone controller assignment is *intended* — `cf_second` stays pinned to stock Lee, `cf231_active` is the vehicle under study — so the override is kept and only the logging was fixed: each drone's log now carries its own resolved config plus `config_source`, `shared_controller`/`shared_ctrl_mode`, and `gains_apply=0` for a drone on a stock controller that ignores `indi_gains`/`pos_gains`. **Still open:** `cf231_active`'s radio log came out all-zero on 2 of 10 flights (correct row count, every value zero, *not* crash-correlated, one drone only, not reproducible) — root cause unknown, mitigated by an in-lab warning at save time rather than fixed; and the ~0.5 s attitude transient at the `apply('scenario')` controller/gain handover (roll +15/−19°, `gyro_x` to −474 °/s) is pre-existing and unrelated to the `usec.reset` bug. |
| 2026-09-14 (35) | **Neural-Swarm2 test suites rewritten and passing; `model.py` (the firmware contract) rewritten too; `dataset.py`/`train.py` deliberately left blocked on decisions.** The two suites and `model.py` all still described the old 987-weight custom network, so their PASS output was meaningless and both training files imported symbols that no longer existed. Rewritten: `firmware_app/host/test_residual_nn.py` (**19 checks**), `tools/residual/model.py`, `tools/residual/test_pipeline.py` (**13 checks**) — all passing, verified against the **compiled firmware** rather than self-consistently (torch, the NumPy reference and the real controller agree to **~1e-6 m/s²** across gated-in, gated-out, zero-velocity and differenced-velocity cases). Three architectural changes drove most of the new assertions: the output is **scalar and Z-only**; `phi_G` runs **unconditionally**, so a lone drone does *not* predict zero (the old suite asserted exactly that, and was actively wrong); and neighbour inclusion is the reference's asymmetric `\|dx\|<0.2, \|dy\|<0.2, \|dvx\|<1.5` gate rather than a distance cutoff (each term tested separately, plus one proving `dz` is deliberately *not* gated). **Four latent footguns found and encoded, none of which crash — all produce plausible wrong numbers:** (1) `RNN` is a process-global static that `controllerOutOfTreeInit()` does not reset, so an earlier upload stays loaded across test instances; (2) `rnn.pred_*` is written only from inside `rnn_predict`, so while the network is not ready the log holds its **last** value rather than zero — harmless from cold boot, visible if `ready` drops mid-session, which a fresh weight upload does; (3) `neighbour_gate()` **passes an all-zero row**, so applying it to a zero-padded array marks the padding as a neighbour sitting exactly on top of the vehicle — the strongest input the network can receive (fixed by `build_mask`); (4) `g_rnn_pred_z` is a global the controller overwrites every tick from any harness, so reading two harnesses' predictions at the end of a test returns the same number twice — cost one false failure before it was spotted. Also added a guard to both suites: the `residual_nn` feature is **off by default**, and without it every prediction is zero and several checks would pass on a network that was never there. **`dataset.py` is NOT rewritten** — it now raises on import with a full explanation rather than a cryptic `ImportError`, because it needs four decisions rather than effort: the target is now **scalar** (so the x/y components of the measured `a_res` have **no predictor in Strategy 2** — a scoping consequence for the comparison, recorded in `docs/18`), `build()` must emit the ground-effect input, **`--z-floor` now contradicts** a network that models ground effect explicitly, and the distance guards are replaced by `build_mask`. The loader checks that used to live in `test_pipeline.py` (peer-minus-own convention, both drones as ego, dropping `a_res == 0`) went with `dataset.py` and are **not covered anywhere right now** — a real gap, recorded in the README rather than glossed over. `docs/13` and `docs/18` updated throughout; `docs/13`'s "software foundation is COMPLETE" banner and its §6/§7 results corrected or marked superseded. Net: **weights that train will fly correctly; training from flight logs is still blocked.** Not on the critical path until C.1 produces logs; blocks C.2. |
| 2026-09-14 (34) | **Reverted the two-draft split to ONE main draft (Strategies 0-4); the advanced-strategy prose archived verbatim, not deleted.** Operator's decision: the conditional-build machinery was more complexity than the problem warranted, and there should be a single main thesis draft scoped to the strategies the comparative study will actually include. `\ifadvancedtier` removed from all five chapters and from `preamble_draft.tex`; `thesis_draft_minimum.{tex,pdf}` and `thesis_draft_advanced.{tex,pdf}` deleted; `thesis_draft.tex` is the single draft again, now permanently covering Strategies 0-4. **No work lost**: the full Strategy 5/6/7 prose (ch4's three sections, ch2's complete residual-RL section and MPC paragraph, the three Table 4.1 rows, the seven-strategy summary wording, and the one-line references from ch1/ch3/ch5) moved verbatim into `docs/thesis/reserve/advanced_strategies_reserve.tex` — six labelled blocks, each recording its origin and what re-insertion would require. That file is never built and never `\input`. **Two real defects found and fixed during the rework**: (1) ch4's opening sentence still read "Strategies 0--7 (eight controllers...)" while its own table listed only 0-4 — that line had never been conditionalised, so the Minimum PDF shipped with a self-contradiction; now reads "Five controllers". (2) `sec:ctrl-common` still grouped Strategy 7 with the geometric-loop strategies and 3/5/6 as structurally different, a leftover from the (32) renumbering. `thesis_draft.pdf` now 41 pages, builds clean, zero undefined citations or references, no residue of the advanced material. |
| 2026-09-14 (33) | **Built the two thesis draft versions: `thesis_draft_minimum.tex` (Strategies 1-4) and `thesis_draft_advanced.tex` (all 7).** `ch4_control_architectures.tex` gained an `\ifadvancedtier` LaTeX flag (defaults true, so the original `thesis_draft.tex` and `ch4_standalone.tex` are byte-unaffected) gating the three Advanced-tier strategy sections, their table rows, and the summary's strategy count. Chapters 1, 2, 3 and 5 are shared unmodified between both builds, confirmed strategy-agnostic during (32)'s numbering pass. Minimum builds to 43 pages with zero Strategy 5/6/7 mentions (verified via `pdftotext`); Advanced builds to 44 pages, content-identical to the pre-existing `thesis_draft.tex`. Both build clean, zero undefined citations/references. This closes the "two separate draft versions" item from the 2026-09-14 supervisor meeting follow-through. |
| 2026-09-14 (32) | **Resolved the strategy-numbering inconsistency flagged in (31) — `docs/01`'s scheme adopted everywhere.** `ch4_control_architectures.tex` used its own numbering (1=uncompensated baseline, shifting INDI/Geo+NN/FBL/Hybrid up to 2/3/4/5, MPC=6, residual RL split as 7a/7b) — different from `docs/01`/`docs/18`/`docs/19`, which already agreed with each other (1=Pure INDI ... 7=Geometric+Residual RL, with an unlisted "Strategy 0" baseline outside the 7-method count). Renumbered `ch4` to match: baseline is now Strategy 0, INDI/Geo+NN/FBL/Hybrid are 1/2/3/4, residual-RL-cascaded is 5, MPC is 6, residual-RL-geometric is 7 — and reordered the chapter's sections so Strategy 6 (MPC) now follows Strategy 5 rather than preceding both residual-RL variants, matching canonical order. Also corrected a factual error introduced while removing the substrate framing in (31): Strategy 7 (residual RL on the geometric baseline) genuinely does share the geometric position/attitude loop with Strategies 0/1/2/4 (its own "What differs" paragraph says so) — it had been wrongly grouped with the structurally-different strategies (FBL, MPC, residual-RL-cascaded). Found and fixed the same stale numbering **and the same substrate/tiered-fairness framing (31) was supposed to remove but had missed** in `ch1_introduction.tex` (two places) and `ch5_experimental_setup.tex` (three places) — those chapters weren't in the file list checked for (31), since that pass was scoped to the tier-language sweep, not the substrate framing specifically. All five chapter files now describe every strategy as its own controller under uniform experimental conditions, with only factual similarity notes (INDI/hybrid share an attitude law) kept as local remarks. `docs/18` and `docs/19` were already using `docs/01`'s numbering, no change needed there. Thesis rebuilds clean: 44 pages, zero undefined citations or references. |
| 2026-09-14 (31) | **Dropped the "shared control-law substrate" fairness framing from `ch4_control_architectures.tex` and `19`, per operator's explicit correction.** Chapter 4 previously grouped strategies into a "Core" set claimed to share one control law (the strongest form of the fairness argument) versus "Optional"/"structurally different" strategies compared under a weaker standard — three separate passages built on this (the chapter intro, `sec:ctrl-common`, and `sec:ctrl-fairness`'s "what is held constant"/"what this does not guarantee" paragraphs), plus the chapter summary. Operator's correction: each of the seven strategies is its own controller; the comparison's fairness comes from the experimental conditions held constant uniformly across all seven (airframe, trajectories, formations, residual definition, frozen gains), not from asserting that some subset shares an implementation substrate. Real similarities (pure INDI and the hybrid share an attitude law, differing only in which residual term is active) are now stated as clarifying, local remarks, not a formal comparability tier. `docs/19`'s C1 section had the same table and was rewritten the same way. Chapter still builds clean, 44 pages, zero undefined citations/references. **Separately flagged, not yet resolved**: while making this change, confirmed `ch4`'s own strategy numbering (1=geometric baseline, 2=pure INDI, 3=Geo+NN, 4=FBL+NN, 5=Hybrid, 6=MPC, 7a/7b=Residual RL) does not match `docs/01`'s numbering (1=Pure INDI, 2=Geo+NN, 3=FBL+NN, 4=Hybrid, 5=ResidualRL, 6=MPC, 7=Geo+ResidualRL) or `docs/19`'s implied numbering (which prepends an unlisted "Strategy 0" baseline). This is a real, pre-existing cross-document numbering inconsistency, independent of the substrate question, that needs a decision before any further cross-referencing between `01`/`19`/`ch4` can be trusted at the strategy-number level. |
| 2026-09-14 (30) | **Strategy-tier split simplified from three tiers to two; docs/01/03/15/19/05 updated; a real, pre-existing numbering inconsistency found in `ch4_control_architectures.tex` vs `docs/19`, not yet resolved.** Operator's decision: Minimum = Methods 1–4 (Pure INDI, Geometric+NN, FBL+NN, Hybrid/NA-INDI), Advanced = Methods 5–7 (Residual RL, Light Learning-MPC, Geometric+Residual RL) — replacing the old Minimum(1-3)/Ideal(1-5)/Super-perfect(1-7) framing. Rationale: the hybrid now has a numerically-verified, unflown Rust port (`stabilizer.controller=7`), so it's no longer a stretch goal. Updated: `01`'s table and target-levels section; `03`'s contribution list (now "at least four controllers"); `15`'s open-question resolved; `19`'s open TODO resolved and C2 reframed as a guaranteed Minimum-scope contribution, not contingent; `05`'s goal list and phasing paragraph (was missing the hybrid entirely). **Found but not fixed**: `ch4_control_architectures.tex` groups strategies by *shared control-law substrate* ("Core 1,2,3,5" / "Optional 4,6,7") — an axis orthogonal to the tier split — and its grouping directly conflicts with `docs/19`'s own "Strategies 0,1,2,4 are one control law, 3 and 6 are structurally different" classification (ch4 puts 3 in the shares-substrate group and 4 in the different-substrate group; `19` says the opposite for both). This predates tonight's tier-split work and needs a substrate-vs-tier reconciliation before Chapter 4 can be cleanly forked into Minimum/Advanced draft versions — flagged for the operator, not resolved unilaterally. Chapters 1, 2, 3, 5 need no forking (strategy-agnostic or already written at the methodology level); only Chapter 4 requires real restructuring for a two-draft split. |
| 2026-09-14 (29) | **Neural-Swarm2 exact-architecture port implemented (`residual_nn.rs`, `a45ea03`) — does not currently fit in RAM, and per operator's explicit standing instruction is NOT to be fixed by modifying the network.** Ported `phi_Net`/`rho_Net` directly from the vendored reference (`crazyswarm2/crazyflie_sim/crazyflie_sim/backend/neuralswarm.py`): `input_dim -> 25 -> 40 -> 40 -> H=20` / `20 -> 40 -> 40 -> 40 -> 1`, all five weight sets (`phi_S`/`rho_S`, `phi_L`/`rho_L`, `phi_G`), the exact asymmetric `dx`/`dy`/`dvx` proximity gate, and the always-on ground-effect term — replacing the old, differently-shaped 987-weight custom net entirely. One correction to an earlier note: there is no spectral normalization visible in the vendored inference file; that claim was carried over from an unverified summary. **19297 weights (~77 KB), not 987 (~4 KB) — a real `make DRONE=bl` link overflows RAM by ~39 KB, confirmed not estimated.** Presented three fix options (drop `phi_L`/`rho_L`; also drop `phi_G`; leave as-is); **operator's decision: leave the architecture exactly as ported, and do not touch it to fix the overflow, ever — "it should work out of the box as it is."** Consequence: does not currently build into a flashable firmware; whatever eventually resolves this has to come from elsewhere in the build, not from the network. `firmware_app/host/test_residual_nn.py`, `tools/residual/test_pipeline.py` and `tools/residual/train.py` all still validate/target the old shape and were not updated — their PASS/FAIL output is stale until rewritten. `docs/18` updated to match. |
| 2026-09-14 (28) | **NA-INDI faithful port numerically verified against the reference's own compiled C — 5/5 test vectors match to ~1e-9.** Built a throwaway copy of `~/Desktop/NA-INDI-firmware` outside the repo (kept strictly read-only, per convention — full reproduction steps in `firmware_app/host/naindi_reference_build_notes.md`; the compiled `.so` itself is not checked in), compiled their own `controller_lee.c` into its own Python bindings, and ran hand-picked non-trivial states (hover, position error, combined roll+velocity error, 90° yaw with asymmetric RPM, a fully aggressive multi-axis case) through both implementations via `firmware_app/host/test_naindi_reference.py`. Mass/J/kt — the one deliberate, physically-necessary deviation — were pinned identically on both sides for the test via a small test-only hook (`naindi_test_set_j`, defaults to the real per-platform inertia when never called), isolating "is the algorithm ported correctly" from "do the two projects fly different hardware". **A 4th deviation surfaced during validation**: the port only implements the reference's `modeAbs` yaw-source branch, not its velocity/disable branches — consistent with this project's existing convention that Mode E/HLC setpoints are always absolute. Found because the two non-zero-yaw test cases needed `setpoint->mode.yaw=modeAbs` forced on the reference side to match at all. Also fixed a host-only artifact along the way: the `usecTimestamp()` stub originally used real wall-clock time, and a fast host test loop could call it twice within the same microsecond, making `dt=0` and the reference's own un-guarded `(omega-omega_prev)/dt` divide into NaN — not a real discrepancy (real hardware never sees `dt=0` at a 2 ms tick), fixed by making the stub advance a fixed 2000 µs per call instead. |
| 2026-09-14 (27) | **NA-INDI faithful port implemented and building — new `stabilizer.controller=7`.** Supervisor feedback judged the INDI oscillation unacceptable as a known limitation; decided to port Cobo & Briesewitz's `controller_lee.c` bit-for-bit rather than continue iterating on our own INDI, so their published gains/filters transfer directly. New, fully independent controller slot `ControllerTypeOot2`/`CONFIG_CONTROLLER_OOT2` (patch `naindi_controller_slot.patch`) alongside the existing `=6` (our geometric/INDI) — separate enum value, dispatch row, Rust module (`firmware_app/src/naindi.rs`), no shared state. Direct source reads (not summaries) resolved two open questions from the initial investigation: (1) the reference's own `RATE_DO_EXECUTE(ATTITUDE_RATE,tick)` gate genuinely self-throttles to 500 Hz — there is no 500-vs-1000 Hz rate bug in their code itself (the real preserved quirk is a fixed-dt position loop vs. a wall-clock-measured-dt attitude-INDI term, ported as-is per instruction); (2) a synthetic hover test caught two real porting bugs before any hardware was involved — `a_imu` must read `state->acc` (EKF world-frame, gravity-compensated) not `sensors->acc` (raw body IMU), and the position loop's feedforward needs `GRAVITY_MAGNITUDE` added to setpoint acceleration's z-component, without which commanded hover thrust converged to ~0 instead of ~mass·g. Also added `sensorData_t.gyroNoLpf` to our own `crazyflie-firmware` (patch `naindi_gyro_no_lpf.patch`, ported verbatim from the reference's own firmware fork) so the port's main angular-rate signal — used throughout the control law, not just the INDI finite-difference term — reads the identical unfiltered gyro the reference does, per the operator's explicit "no exceptions apart from mass/inertia/kt" instruction. Builds clean on `bl`/`std`/`upgrade` and the host target; existing residual-network test suites (22 checks) unaffected. Never flown; no numerical validation yet at this point (done same day, see (28)). |
| 2026-09-13 (26) | **Offline notch-filter bode/phase analysis — CONCLUSIVE: the notch approach is structurally flawed, not just buggy.** `experiments/analysis/notch_filter_bode.py` ports the `Butterworth2`/`NotchFilter` coefficient formulas directly from `firmware_app/src/lib.rs` and computes the real discrete frequency response at the true 1 kHz tick rate. Finding: the attitude loop's own natural frequency (`ωₙ = √kr = 7.80 Hz` at the locked `kr=2400`) sits almost exactly inside the notch's rejection band (`f0=6.9`, `bw=3.0`, roughly 5.4–8.4 Hz) — at 6.88→6.98 Hz the phase swings from −89° to +85° in under 0.1 Hz, and even at the loop's own 7.80 Hz bandwidth the notch still applies +58° phase and halves the signal magnitude. **The "shake" this notch was designed to remove likely IS the attitude loop's own closed-loop dynamics (already suspected from the stage-2e `kr`/`kw` bisection result), not an independent external resonance** — so notching it doesn't move the loop away from the problem the way lowering `kr`/`kw` does, it corrupts the loop's own feedback signal with large phase distortion exactly at its operating point. No retuning of `notch_f0`/`notch_bw` can fix this while `kr`/`kw` keep `ωₙ` at 7.8 Hz, since there is no frequency gap between the shake and the loop bandwidth to place a notch in. **Decision: the notch-filter approach is abandoned for this problem.** The coordinated `kr`/`kw` + `pos_gains` retuning line from 2026-09-11 (best result so far: attempt 4, `483/76` + geometric `pos_gains`, peak oscillation 6.7°) is the path forward instead. |
| 2026-09-13 (25) | **Notch fix (`bc7190c`) flown for the first time — CRASHED, worse than the 2026-09-11 crash, zero log data.** Most of the session before that was consumed by a genuine hardware/tooling problem: `ros2 launch` hung silently with no error. Traced through several dead ends (stale ROS processes on an unrelated machine, `cf_second` blocking the launch, recent yaml/code changes reviewed and ruled out) to the real cause — the drone had been flashed with the **wrong firmware platform** (standard CF2.1 onto the brushless airframe), and a diagnostic (`bl.start_bootloader(warm_boot=True)`) had also actively commanded it into bootloader mode rather than just detecting it, compounding the confusion. Resolved by an explicit `make DRONE=bl` rebuild (confirmed via the build's own "Build for the cf21bl platform!" banner) and a correctly-timed reflash. Once connected, the actual notch test ran at `stabilizer.controller=6`, `ctrl_mode=3` (full INDI — an earlier attempt at `ctrl_mode=0` was caught and corrected first, since the notch filter only touches the attitude-INDI path), `notch_en=1`. **Crashed worse than 2026-09-11's original crash, and nothing was logged** — the crash was violent enough the radio link/log pipeline never completed, the first time this project has had a crash with zero data. `notch_en` reverted to 0 (`crazyswarm2` `3958a4e`); **do not re-enable without real offline analysis**, which was flagged as needed back on 2026-09-11 and still has not been done. Drone inspected after, no physical damage. Full account: `docs/lab_sessions/2026-09-13.md`. |
| 2026-09-13 (24) | **Architecture decision: `cf_second` permanently pinned to stock Lee, regardless of platform.** Only `cf231_active` is ever the controller under study (geometric/INDI, and later NA-INDI/IL-NDI once integrated) — `cf_second` exists purely as the other formation member / downwash source. This removes the whole cross-platform gain-sharing risk that caused the 2026-09-12 mixed-platform crash by construction, not by fixing the sharing: stock Lee is a separate firmware controller and never reads `indi_gains`/`pos_gains` at all, so there is nothing left to share incorrectly. Consequence: the planned brushless swap for `cf_second` is no longer needed for this reason — it can stay standard CF2.1 indefinitely (a swap might still be worth doing later for other reasons, e.g. a third actively-controlled robot for the B-scenarios, but not for this). Implemented in `crazyswarm2` `bb88e73`: `cf_second`'s `firmware_params.stabilizer.controller: 5` pin kept, the now-pointless per-robot mass/kt override removed (stock Lee never consumed it). No code changes needed — the per-robot override mechanism fixed the day before (`fdfc640`) already re-pushes this correctly. |
| 2026-09-12 (23) | **Desk-time follow-up to (22), same day: notch crash root-caused and fixed (unflown), the mixed-platform crash story corrected with a bigger second bug, and both web pages republished.** **(1)** Root-caused the 2026-09-11 notch crash: the 2026-07-29 fix hoisted `alpha_meas`/`alpha_ref` notch-filter warm-up out of the mode-gated branch to kill a controller-handover cold-start transient, but missed a third chain — `tau_current`'s own Butterworth pre-filter (`bw_tau_x/y/z`) only started updating once `filt_tau`/`notch_en` went true, which lands on the same tick as the ctrl_mode handover in every flight script. Same fix pattern applied in `firmware_app/src/lib.rs` (`flying_robot_course` `bc7190c`); `DRONE=bl`/`DRONE=std`/host builds all verified clean and behavior-preserving at defaults — **not yet reflashed or flown**, needs a real `notch_en=1` test next session. **(2)** While generalizing the per-robot override fix from (22), found a second, bigger bug: `apply()`'s broadcast in `formation_flight.py` also carries `indi_gains.mass`/`kt1-4` from the shared `all:` block, which was silently wiping `cf_second`'s per-robot mass/`kt` override on every single `apply()` call — including the very first one before takeoff. `cf_second` almost certainly flew the crashed mixed-platform flight on brushless's thrust model (mass 0.041 vs its real 0.0364 kg, `kt`≈4.1e-10 vs its real ≈1.4e-10, ~2.8× off), not its own values as intended, despite the override having been added earlier that day specifically to correct for it. This plausibly matters more than the shared-config-drift explanation already on record — both are likely real contributors. Fixed generally: `load_per_robot_overrides()` now re-reads and re-pushes ANY per-robot `stabilizer`/`indi_gains`/`pos_gains` key after every broadcast (`crazyswarm2` `fdfc640`), so per-robot config always wins last, for any future key, not just this one. **This bug explains `cf_second` (standard)'s own instability, not `cf231_active` (brushless)'s crash directly** — brushless's config was never touched by it, since it's the platform the shared config is tuned against; its crash 2s after leaving the ground, with a real `a_res_z` spike of 8.1 m/s² at the moment of divergence, reads as collateral damage from the standard drone thrashing directly above it, not a separate fault of its own. **(3)** Republished `Controller Validation Card` and `Two Tracks to the Thesis` with all of the above; committed and pushed (`flying_robot_course` `75213f2`). |
|---|---|
| 2026-09-12 (22) | **H0 partition run (inconclusive), first-ever 2-drone flight achieved (5 real bugs found and fixed), mixed-platform geometric attempt crashed both drones (root-caused to shared-config drift, not the attitude gain).** Full account: [`docs/lab_sessions/2026-09-12.md`](lab_sessions/2026-09-12.md). **(1)** `ctrl_mode=2` (attitude-INDI-only) and `ctrl_mode=1` (position-INDI-only) both crashed on hover alone (119.7°/179.3° peak) — far worse than full INDI, the mildest config throughout. Conclusion: the two INDI sub-loops are too coupled for this kind of isolation to localize the trajectory-tracking shake; full-INDI sanity check confirmed unchanged (hover clean, figure8 shake reproduced at the same magnitude). **(2)** First 2-drone flight: found and fixed real bugs in `formation_flight.py` — missing `pos_gains` selection for `ctrl_mode=0` (the exact 2026-09-09 root cause, never ported to this script), a mid-air controller-*identity*-switch risk (ramp always on OOT-6 then switches to the target — fine within our own `ctrl_mode`s, dangerous switching to/from a genuinely different controller like stock Lee), no hover support at all (always required a CSV that was never exported for "hover"), arming gated behind an unused `--brushless` flag that wasn't passed (CF21BL silently never armed while the auto-arming standard drone took off fine), a hardcoded dev-machine log path that crashed the whole flight *before landing* on a different lab machine, and a landing collision for vertical/stacked formations (broadcast `land()` closed two drones to zero XY separation on descent — confirmed in a log, one drone ended at 166.6° roll). All fixed; the landing fix ports the already-proven spread-then-rate-matched-descent pattern from `run_formation.py`. **(3)** First fully clean stock-Lee 2-drone flight showed a real physical signature: the lower (downwash-receiving) drone's roll std was 17.9° vs the upper drone's 0.75° — stock Lee has no interaction-force awareness. `a_res` was exactly 0 for both, expected (only computed under our own OOT controller). **(4)** Switching both drones to our OOT geometric controller crashed both — the standard drone oscillated almost the whole flight (peak 179.5°), and that instability directly crashed the brushless drone stacked below it (a real `a_res_z` spike of 8.1 m/s² at the moment of divergence). Root-caused carefully, not assumed: `kr_geo`/`kw_geo` is verified bit-identical (via git history) to the exact value that flew clean on standard back on 2026-07-02 — the actual cause is that everything *else* in the shared `all.firmware_params` block (`fc_bw`, `clamp_en`, `res_fc`/`res_clamp`, `tau_xy_max`, `thrust_max`, notch settings) has been iterated on and validated only against brushless since July, and standard silently inherited the whole current configuration the moment it was re-enabled. **Decision: next session switches the second drone to brushless** (matches the project's own "2x identical brushless" protocol), removing this failure mode entirely, then validates uSD logging on both cards before real C.1 data collection begins. |
|---|---|
| 2026-09-11 (21) | **First live run of the controller-validation card. Stage 1 closed clean; stage 2 made real progress (attitude oscillation 88°→7°) but no clean pass; a real crash and a real trajectory-export bug were both found and handled.** Full account: [`docs/lab_sessions/2026-09-11.md`](lab_sessions/2026-09-11.md). **(1)** Found and fixed a genuine `export_poly4d.rs` bug before any flying: the rest-to-rest wind-up ramp searched for the shortest feasible duration from a 0.65s floor, short enough that a degree-7 Hermite polynomial rang (peak accel ~7 m/s², sign-reversing) — matched exactly to a violent ±53° roll oscillation at trajectory start on circle/figure8 kt=0.05. Fixed by raising the floor to 1.2s (`976352d`/`bfa4db9`), confirmed in flight. Also re-examined and reconfirmed `--rest-to-rest` (on-path, no wind-up wrap) as measurably worse even with more Richter redistribution iterations — real degrees-of-freedom limitation (sparse waypoints can't absorb a full rest-to-lap-speed swing without bowing off path), not a bug, not attempted as a fix. **(2)** Stage 1: geo hover/circle/figure8 + restored-config INDI hover all PASS. **(3)** Stage 2a (`res_fc=80`/`res_clamp=10`) and 2b (`filt_prewarp`/`filt_dt_us`/`fc_bw=206` no-op) both closed clean. **(4)** Stage 2c (`fc_bw=100`) made things worse (hover regressed PASS→FAIL, circle peak roll 27.5°→69.2°) — reverted; NOT pursuing lower `fc_bw` further. **(5)** Stage 2d (notch, `notch_en=1`) **CRASHED** — stable hover to t=6.4s then 88.7°/66.6° divergence and altitude collapse within 0.7s, a real impact (drone inspected after, no damage). The 2026-07-29 cold-start-transient fix did NOT resolve whatever makes the notch unsafe; root cause unknown, `notch_en` reverted to 0, do not re-enable without offline analysis. **(6)** Stage 2e (shake-band, 4 attempts): uncoordinated `kr/kw=987/109` alone diverged (pos_gains/attitude-bandwidth cascade mismatch, a real mechanism); a sim attempt to de-risk this failed to reproduce the hardware failure at all (idealized actuators/estimator, sim ruled unusable for this question); coordinated `kr/kw=987/109`+matched pos_gains passed hover cleanly but left figure8 unchanged (17.6° vs baseline 17.7°) — reconciled as still being on the unstable side of a documented sharp stability boundary, not evidence against the gain-magnitude hypothesis (stock defaults, ω_n=1.59Hz, are independently known stable); bisecting to `kr/kw=483/76` dropped hover peak to 11.4° but broke position holding; switching to geometric's own pos_gains (40/8/30/10) fixed position holding and dropped the peak to 6.7° but left a small new hover growing trend. **No configuration reached a clean pass.** Session ended reverted to the confirmed-clean locked point (`kr=2400`/`kw=170`, pos_gains 64/5/48/7). EKF-phase-lag theory raised and explicitly rejected (INDI flies clean on other drones sharing the same EKF/mocap pipeline). **Next: root-cause the notch crash offline; run the card's own H0 partition (`ctrl_mode=2` then `1`), never actually done this session; second brushless drone being prepped in parallel for the first 2-drone flight, gains not yet frozen.** |
| 2026-09-11 (20) | **DShot RPM investigation moved from plan to executable prep, at the operator's request.** `docs/23_DShot_RPM_Investigation.md` reads the full 2026-07-15/16/18 history straight from the primary log and states the genuinely open question precisely: the 07-16 deck fix was real (ended an outright crash), but the mechanism explanation attached to it was revised twice (07-18 H1a + retraction) **entirely on deck data** — DShot was never re-tried at the gains that revision actually produced. Added, all default-off and verified: **(1)** `indi_gains.rpm_source` — runtime switch between the optical deck (0, default, unchanged) and DShot bidirectional telemetry (1), replacing what was previously a compile-time-only choice (reflash required to A/B). Builds clean for `DRONE=bl` and `DRONE=std`; host build unaffected (`rpm_get_all` is stubbed for the simulator regardless). **(2)** `usd_dshot_investigation_config.txt` — logs both RPM sources concurrently (35/40 vars) so a single flight settles agreement/lag regardless of which one fed INDI; `decode_usd_log.py`'s rename table completed (`motor.m2/3/4_rpm`, `indi.dt_us` were missing). **(3)** `investigate_dshot_rpm.py` — reports the four numbers the July investigation actually used (gyro σ+FFT, `tau_*` σ+FFT, deck-vs-DShot bias and cross-correlation lag, `dt_us` sanity), verified against synthetic data with a known injected delay — which caught and fixed a real sign-convention bug in the lag calculation before it could mislead a real result. **Explicitly parked for after the controller-validation card and gain freeze, before two-drone flights** — nothing flown, nothing on the C.0 critical path. |
| 2026-09-10 (19) | **Desk day. The NA-INDI authors' code arrived; comparing it against ours produced two real bugs on our side and one strong lead on the oscillation.** Survey: [`22`](22_NA_INDI_Repo_Survey.md); pages: [comparison](https://claude.ai/code/artifact/45a5f215-c23a-4a5b-8d66-45855c403fe0), [flight card](https://claude.ai/code/artifact/c3deffc6-a90e-43ac-8a1c-f21f086106df). **(1)** Their four "controllers" (Lee / INDI / IL-NDI / NA-INDI) are one file switched by two bitmasks, and **their INDI is algebraically identical to ours** — both reduce to `J·α_ref + ω×Jω − τ_dist`; ours is faithful Tal & Karaman, theirs the disturbance-observer arrangement. **Ours is not wrong.** What differs is conditioning. Located their firmware (not `upstream_lee` — `Tupryk/dev-leePayload`), cloned read-only to `~/Desktop/NA-INDI-firmware`. **(2) Filter bug:** `controllerOutOfTree` runs at **1000 Hz**, not 500 — `stabilizer.c` calls it ungated — while every filter was initialised with a hard-coded `DT=0.002`. And our discretisation has **no pre-warping**, warping ~1.9× on top. Net: `fc_bw=60` really gives ~206 Hz, and `notch_f0=6.9` really sat at **13.8 Hz — so the notch aimed at the measured peak never touched it**, which reopens "notch tested + failed". Both fixed behind `filt_dt_us` / `filt_prewarp`, defaults unchanged. **(3) Gain comparison:** converted to a common basis our attitude gains are **8.2× stiffer** than theirs, and **our loop's ω_n is 7.80 Hz while the documented shake is 6.3–7.9 Hz** — the entire historical `kr` ladder (1500–2800) sat inside that band, and it explains why the shake never responded to `kw`. Strong coincidence, not proof; queued as stage 2 (e). **(4)** Implemented `res_fc`/`res_clamp` (condition `a_res` on both sides before differencing — the one thing NA-INDI does that we never did). **Every change is default-off and verified byte-identical at defaults across all four `ctrl_mode`s (0/100 samples), so the validation flights are unaffected.** Flight plan restaged: stage 1 = 3 geometric + 1 INDI on the restored config alone; stage 2 = the new switches, one variable per flight. Also ruled out the gyro-source hypothesis (~5.8° total phase lag at 6.9 Hz, too small) and corrected two published numbers of my own (`fc_bw=120` not 17.5 for the no-op; the yaw-filter "bug" that is actually a deliberate 10 Hz design). |
| 2026-09-08 (17) | **Sim-only follow-up to the 09-07 lab session — three real bugs found and fixed in `run_formation.py`/the sim server, none hardware-relevant but all now de-risking the next lab session.** (1) The sim server never published a `/{name}/pose` topic at all — real hardware does (radio `pose` log config), so every position-reading tool that assumed sim parity (`c.get_position()`, the runner's own relative-geometry check) silently read `[0,0,0]` forever. Added the missing publisher to `crazyflie_sim/crazyflie_server.py`, fed from the SIL controller's real internal state. (2) `run_formation.py`'s landing was a single broadcast `land()` — same duration for every robot regardless of height, so any vertical-stack scenario (A1/A2/A5/A6/A7/A8/B1/B2/C4) closed separation to exactly zero, and even rate-matching the descent wasn't enough on its own: two robots converging to the *same* ground target inevitably cross the *same* XY column on the way down. Fixed by spreading any XY-overlapping robots onto a small circle around their shared point (while still at very different flight heights, so it's safe) before the rate-matched descent begins. (3) The end-of-flight verification report was measuring position *after* landing completed and comparing it to the *pre-landing* commanded separation, always reporting a large bogus error; fixed to snapshot at the actual end of the commanded trajectory. Also fixed the 2-drone sim roster's `initial_position` (`cf_second` was spawned directly above `cf231_active`, sharing XY — changed to a lateral floor offset, matching how the 3-drone roster already does it) and diagnosed a wiggle that compounded across repeated same-server runs (traced to the controller's position-integrator only resetting on thrust<0.05N, which a chained sequence of runs without a real disarm cycle never triggers — the fix is to restart the sim server between test runs, which also mirrors real flight practice). All 9 two-drone scenarios (A1–A8, C4) now confirmed working in sim across both controllers; the 3-drone (B1–B3, C1–C3) and 1-drone (C5) scenarios are written up with correct commands but not yet run. Full command reference: [`docs/lab_sessions/sim_scenario_commands_2026-09-08.md`](lab_sessions/sim_scenario_commands_2026-09-08.md). None of this touches the still-unflown C.0 ladder — the decisive A1 residual-sign flight has not happened on hardware yet. |
| 2026-09-07 (16) | **First real C.0 lab session — `simple_flight.py`'s first-ever hardware flight, one real bug found and fixed, one genuinely new gap found (geometric was never independently tuned for this platform).** Full detail: `docs/lab_sessions/2026-09-07.md`. Flashed the three previously-unflown residual fixes; confirmed the `Crazyswarm()` IndexError is not a race bug, just needs the server up first (10/10 pass once warmed). First hover (accidentally under INDI) reported oscillation; switching to geometric and analysing the CSV found a real, growing (not the historically-known bounded) attitude oscillation. Root-caused via a complete parameter diff against the frozen `finalized-version-for-INDI-project` branch: **`clamp_en` was the only difference** -- every gain, filter toggle, mass and `kt` value is identical. A same-session attempt to "fix" it by restoring the tilt clamp (`11->15`) was the wrong direction (operator confirmed hover/figure8/oval/circle/inverted-loop all flew fine at `clamp_en=11` historically, with a bounded oscillation) -- reverted back to 11, so INDI's config is now byte-identical to the flight-proven state again. Separately, **caught that geometric was silently running the standard-27g-platform's hardcoded attitude gains this whole project** (`KR_X=0.010` etc.), never independently tunable -- added a proper `kr_geo`/`kw_geo` pair (own firmware wiring, two rounds of correction: first version didn't reach geometric's actual torque law at all, second version was ~5 orders of magnitude too large from copying INDI's differently-scaled gains). Geometric genuinely needs its own tuning ladder now; INDI needs nothing further pending confirmation the `clamp_en` revert restores the known-good behaviour. Also added `debug/lab_run.sh` (auto-logs any lab command's terminal output for cross-machine review) and `docs/lab_sessions/` as a dedicated, dated log directory going forward. |
| 2026-09-06 (15) | **Closed out the speed sweep: bumped the client timeout and re-ran all 19 not-yet-clean cases from (14).** Result: 9/19 now clean PASS; 4/19 (B1/B2 at speed=0.1) still coverage-capped at ~72-75% even after the bump, likely because B1/B2's near-vertical 3-robot stack makes the downwash-interaction model per-step more expensive than C2's coplanar spread (which DID reach 100%) -- not chased further, formation geometry within the captured window is unaffected either way. **New finding, not previously visible:** on long (~110s), very slow (0.1 m/s) circle-paced trajectories, the **geometric controller only** shows a real tail-end excursion -- tilt to 9-15°, flown speed overshooting commanded by 2-4x in the last ~15% of the flight. Confirmed genuinely flown (the commanded curve itself decelerates smoothly to exactly 0, checked directly) and controller-specific (INDI on the identical case is completely clean, 0.1° tilt). Reproduced in A2, A5, B1, C2's geometric runs; fades with speed (B1/geo: 14.7° at 0.1 -> 5.3° at 0.2 -> 2.7° at 0.3, where the raw FAIL is just the pre-existing B1 EXPECTED superposition value). Formation-relevant separation accuracy stayed at each scenario's normal baseline throughout. **Decision: document as EXPECTED alongside B1's existing combined-wash case, do not chase further** -- a second, now well-characterised instance of "geometric has a real limitation INDI does not share," consistent with the project's central finding rather than contrary to it. A8/geo/speed=0.2's speed-tolerance-only flag was independently reproduced with identical numbers (sim is deterministic), confirming it as a real, narrow, speed-specific curve-tracking characteristic rather than a fluke. Full writeup: `docs/21_Formation_Speed_Sweep_Report.md` §6. **Nothing is left to re-run for this sweep.** |
| 2026-09-06 (14) | **Wrote `docs/21_Formation_Speed_Sweep_Report.md`** — a standalone report consolidating all four speed-sweep scripts/results from (11)-(13): which of the 16 scenarios have a speed axis at all (13 do, A1/C1/C3 don't), the exact CLI commands used for every scenario (including the period/timescale conversion tables for A5/A8/B3/C4), results by scenario, and a precise accounting of what is NOT yet clean. **Correction to an earlier informal estimate of "3 or so" remaining cases: the real count is 19**, in three understood categories -- (1) 4 cases where trajectory duration alone (94-111s) exceeds the client timeout (A2/A5 speed=0.1), (2) 14 cases where a fixed 3-drone per-run overhead truncates low-speed cases (B1/B2/C2 speed=0.1-0.3), (3) 1 speed-tolerance-only case unrelated to truncation (A8/geo/speed=0.2, geometry itself fine). Proposed fix for (1)/(2): bump `run_sim_matrix.sh`'s client timeout and re-run only those 18 cases, not full sweeps; (3) needs its own re-check since a timeout bump won't touch it. Confirms every scenario with a working speed parameter has now been swept -- nothing with a speed axis remains untouched. |
| 2026-09-06 (13) | **Speed sweep now covers literally every scenario in the library, including the 4 paced by --period/--duration instead of --speed -- A1/C1/C3 (pure hover) are the only ones left untouched, confirmed to have no speed-like axis at all.** User asked to also do the remaining scenarios (A5, A8, B3, C4) "at speeds so ALL pass". These have no `--speed` CLI argument -- A5 is paced by `--period` (a real flag), A8/B3/C4 by an internal `duration` builder kwarg that **`run_formation.py` does not expose as a CLI flag at all**. **Two real bugs found and fixed while doing this, not while looking for them:** (1) the sweep script's first pass used `--duration X` for A8/B3/C4 -- `run_formation.py` uses `parse_known_args()` (needed for `--ros-args`), so the unrecognised flag was silently dropped and all 30 of those runs actually flew at the scenario's unscaled default duration every time, which is exactly why every "different speed" row was numerically identical. Caught by noticing that, not assumed. Fixed by using `--timescale` instead (a real flag; confirmed in source it scales `sc.duration` at `startTrajectory(0, timescale=...)` upload time), with timescale values derived the same peaks()-and-scale method used throughout this whole speed-sweep series. (2) Re-running with `--timescale` then produced a **spurious 1204mm RMSE** on the very first case -- traced to `verify_formation_sim.py` itself: its `verify()` function computed the commanded relative geometry as `sc.relative(i, j, t - t0)`, using real elapsed flight time directly as the scenario's own internal curve time, without dividing by `timescale` first. This bug was invisible in every earlier sweep because they all ran at the default `timescale=1.0` and only surfaced now that a scenario is genuinely timescale-driven. Fixed in `experiments/analysis/verify_formation_sim.py` (`(t - t0) / timescale`, and reused the same `timescale` variable for the peak-speed check that already divided by it correctly). The already-running sweep did not need restarting -- the verifier is invoked fresh each time as a subprocess, so the fix applied to every subsequent case automatically; only the one already-recorded case had to be manually re-verified against its existing flight data. **Final results** (`experiments/sim_validation/speed_sweep_A5_A8_B3_C4_results.md`): A5 8/10 clean PASS (2 INCOMPLETE at speed=0.1, same 94.25s/380s-timeout truncation as every other speed=0.1 case in this whole series); A8 9/10 clean PASS (1 case, geo/speed=0.2, flagged FAIL on speed-tolerance only -- flown peak 0.280 vs commanded 0.200, just over the wide +-0.08 m/s band; geometry itself was fine, 2.5mm mean|ez|, same as its neighbouring clean cases -- likely a settle-to-motion transition overshoot specific to A8's `Then(Pause, Line)` curve shape that isn't present in circle/lemniscate paths, and that cleared up by speed=0.3 as the same absolute overshoot became a smaller fraction of a larger target); B3 10/10 clean PASS; C4 10/10 clean PASS. Confirms the "speed barely matters" pattern holds for these four too. |
| 2026-09-05 (12) | **Speed sweep now covers every scenario in the library that has a working `--speed` parameter -- (10)/(11) plus this closes A4, A6, A7, C2, C5 (50 more runs), completing the speed characterisation of the whole formation library.** User asked to make sure all formation-library flights were covered "where it makes sense" -- audited `formations/scenarios.py` and confirmed only these 5 remained with a functioning speed axis: A4 (but only its `--motion line` variant -- the default `lemniscate` motion is paced by `period` directly and never even reaches `--speed`), A6 (extreme, gated behind `--allow-extreme`, its own docstring says "fly last, and only in sim first"), A7 (extreme, dz shrinks to 0.10m by construction, needed `--height 0.5` to keep its 1.10m-offset top vehicle inside the z-geofence), C2 (3-robot leader-follower, needed `--rotate 90` to fit its along-track length inside the x-geofence), C5 (single robot, ground-effect characterisation). The rest of the library (A1, A5, A8, B3, C1, C3, C4) is hover/duration-fixed/period-only-without-a-speed-param and has no speed axis at all -- confirmed by reading each builder in `scenarios.py`, not assumed. Ran via a new `run_speed_sweep_extended2.sh` (0.1-0.5 m/s x {geo,indi} x these 5 scenarios), reusing the same `run_sim_matrix.sh --source-only` pattern. **Result: 48/50 clean PASS, 2 INCOMPLETE** (C2 at speed=0.1 under both controllers, ~67-68% coverage -- same fixed 3-drone-overhead truncation pattern already documented for B1/B2's low-speed cases in (11), not a new issue). Full table: `experiments/sim_validation/speed_sweep_A4_A6_A7_C2_C5_results.md`. Notable numbers: A4(line) and C2 both essentially flat and near-zero across the whole speed range (geo 0.0-2.4mm, indi 0.0-0.1mm) -- speed barely registers, same pattern as every other scenario swept so far. A6 (tightest sane separation, dz=0.10m) stays small despite the tight gap: geo 3.0-3.5mm, indi 0.1-0.7mm. A7 (separation shrinking to 0.10m by the end) has the largest geo error of the whole sweep, 23.9-27.0mm (falling as speed rises), but indi holds it to 0.1-0.4mm. **This confirms the "speed barely matters, INDI reliably cancels vertical downwash" pattern holds across the entire priority + coverage library, not just the handful of scenarios checked so far.** Confirmed working this time: the pairing-bug fix from (11) (`c4b4bc4`) landed correctly for a freshly-sourced sweep -- every case except the two genuine C2/speed=0.1 truncations was recorded automatically, no manual recovery needed (unlike the whole of (11), which predated the fix). |
| 2026-09-05 (11) | **Extended speed sweep to A2/B1/B2 (30 runs) -- covers the two 3-drone I-stack/V-stack scenarios and the 2-drone circle-tracking scenario, completing speed characterisation across the priority library (A3 done in (10)).** Ran `run_speed_sweep_extended.sh` (0.1-0.5 m/s x {geo,indi} x {A2,B1,B2}) via the same `run_sim_matrix.sh --source-only` reuse pattern as the A3 sweep. **Every one of the 30 cases hit the (10)-era `-newer $MARKER` fallback bug and had to be recovered by hand** -- the fix committed in (10) (`c4b4bc4`, restrict glob to `[A-C][0-9]_*.meta.json` + sort by real mtime) was not yet loaded into this already-running bash process, so each case's real `meta.json`/`csv` pair was found manually (`ls -lat`/timestamp match) and verified directly with `verify_formation_sim.py`, never trusted from the fallback. **Results, full table + notes in `experiments/sim_validation/speed_sweep_A2_B1_B2_results.md`:** A2 (2-drone circle-tracking) 9/10 clean PASS, only speed=0.1 incomplete both controllers (~50% coverage -- its 110.9s duration alone exceeds the fixed 380s client timeout, confirmed duration-specific not clock-jump); geo mean error flat 36-39mm, indi flat 0.0-0.2mm across 0.2-0.5 m/s. B1 (3-drone I-stack) and B2 (3-drone V-stack) both show a **fixed per-run 3-drone overhead** that truncates 0.1/0.2/0.3 m/s cases (coverage trends 28%->57%->86% for B1, 29%->54%->84% for B2, controller-independent) purely because the fixed overhead eats a shrinking fraction of the budget as speed rises; by 0.4/0.5 m/s both reach 100% coverage. At those speeds: B1/geo = EXPECTED (bottom-top ~56-60mm, matching the original 2026-08-23 combined-wash-superposition-breakdown finding), B1/indi = clean PASS (cancels it); B2/geo and B2/indi are both clean PASS at every speed where covered -- confirms the original finding that B2's asymmetric offset superposes more robustly than B1's pure vertical stack. Across all three scenarios, **speed barely moves any error number** -- confirms `SPEED_MATRIX.md`'s pattern holds for the whole priority library, not just A3/figure8/circle. |
| 2026-09-05 (10) | **Session close-out: 3-drone `simple_flight.py` confirmed, A3 speed sweep done, `run_sim_matrix.sh` pairing bug fixed.** (1) Ran `simple_flight.py`'s staged-takeoff fix with 3 drones (only 2 had been tested): all 3 pairs show real, positive in-flight separation (cf231_active-cf5 0.600 m, cf231_active-cf_second 0.333 m, cf5-cf_second 1.200 m) -- fix generalizes cleanly, no asymmetry left with `formation_flight.py`'s 2-and-3-drone coverage. (2) Built and ran a dense A3 speed sweep (0.1-0.5 m/s in 0.1 steps x {geo,indi}, 10 runs) via `run_speed_sweep.sh`, reusing `run_sim_matrix.sh`'s `one_run`/`run_with_retry` through its own documented `--source-only` companion-script hook rather than duplicating logic -- requested to characterise "one drone translating through another's downwash" (A3 is exactly that scenario) at varying speed, extending the existing coarse 2-point check in `SPEED_MATRIX.md`. **Result: 10/10 PASS.** Under geometric, mean error is essentially flat 3.3-3.6 mm across the entire 0.1-0.5 m/s range -- speed barely registers, confirming `SPEED_MATRIX.md`'s prior finding on a denser grid. Under INDI, error grows slowly but stays small throughout: 0.1 mm at 0.1 m/s to 0.7 mm at 0.5 m/s. (3) All 10 sweep cases initially came back NO-DATA from `run_sim_matrix.sh`'s own `-newer $MARKER` pairing check (100% hit rate) -- investigated properly rather than assumed: direct reproduction confirmed `find -newer` itself works correctly given a marker that genuinely predates its target, so this is not a find/bfs bug. Most likely cause, specific to this long unattended background job: the sandbox's realtime clock jumping during suspend/resume, visible directly as mid-session date changes (2026-09-03 -> 09-04 -> 09-05) while this exact sweep was running. Not reliably fixable with a clock-based heuristic, so `one_run` now falls back to newest-overall on a pairing miss instead of failing outright, but warns loudly rather than silently trusting it -- every one of the 10 sweep results was individually hand-verified against its real `meta.json`/`record_states` before being recorded, not taken from the fallback blindly. Full results: `experiments/sim_validation/speed_sweep_A3_results.md`. |
| 2026-09-03 (9) | **Closed the last (7)/(8) caveat -- full matrix re-run, 8/8 PASS.** Fixed two remaining issues in `run_simple_flight_matrix.sh` first: the same `find -newer $MARKER` flakiness as `run_formation_flight_matrix.sh` (switched to newest-by-mtime, safe now that `cleanup()` guarantees one server at a time), and the separation check counting misleading preflight ground-state samples (now filtered to airborne-only, z > 0.1 m on both drones). Re-ran all 8 cases end to end: figure8/circle/oval/hover x {1,2} drones, all clean. **Every 2-drone case's min in-flight separation is a real, positive number**: figure8 0.493 m, circle 0.197 m, oval 0.481 m, hover 0.494 m -- no more near-zero climb-together window anywhere. `simple_flight.py` is now fully validated for both single- and multi-drone use, and its own docstring warning updated to reflect the fix + re-validation rather than "do not use". |
| 2026-09-03 (8) | **Fixed the (7) collision-risk window.** Replaced the broadcast `allcfs.takeoff(targetHeight=args.height)` with a per-drone `c.takeoff(targetHeight=initialPosition.z + args.height)`, straight to each drone's own final height -- same pattern `formation_flight.py` already uses. Verified directly against `record_states` (not the matrix's own flaky separation check): by `t=9.3s` the two drones are already 0.527m apart, matching their intended 1.0m/1.5m targets, versus climbing together to the same height for several seconds before. Separation stays 0.5-0.9m through the rest of the flight. One thing NOT to overclaim: the raw min-separation scan still reads 0.000m during `t=0.6-6.2s`, but that's *before* takeoff is ever called (both drones stationary, unarmed, on the ground during preflight/EKF-settle) -- present identically before and after this fix, unrelated to what changed, not a real collision risk since nothing is moving. The actual danger (climbing together mid-air) is resolved. `simple_flight.py`'s own docstring warning updated accordingly; still marked "not yet re-validated via the full matrix" until `run_simple_flight_matrix.sh` is re-run end to end (this was a single spot-check, not the full 8-case sweep). |
| 2026-09-03 (7) | **`simple_flight.py` sim-validated single-drone; found a real multi-drone collision-risk window, NOT a false alarm.** Built `run_simple_flight_matrix.sh` (figure8/circle/oval/hover x {1,2} drones). Found and fixed two real bugs before it could run at all in sim: (1) preflight called `flight.py`'s `_firmware_idle_reset` unconditionally, which touches `traj.mode` -- a Mode-D-only param the sim backend never declares, and this script is Mode E only by design, so the cleanup didn't even apply to it; wrapped in try/except in `simple_flight.py` (not `flight.py`, which stays frozen per standing instruction). (2) `oval_mode1_kt0.008.csv` (the script's own default kt) didn't exist; exported it, and found the install tree needed a rebuild to pick up a source-tree-only new file (classic `--symlink-install` gap, not a bug). **Result: all 4 single-drone trajectories PASS cleanly** (after also bumping the 2-drone-cases-only client timeout 150s->220s for the same "flight completed, outer timeout cut off final cleanup" pattern as (6) -- confirmed by re-running just those 3 and getting clean `rc=0`). **But checking actual inter-drone separation (not in the original matrix -- added after noticing the built-in check never fired due to the same `find -newer` marker flakiness as `run_formation_flight_matrix.sh`) found a real problem**: two drones sharing the same (x,y) in the roster come within ~0-1mm of each other for several seconds during takeoff, because `simple_flight.py` calls `allcfs.takeoff(targetHeight=args.height)` as ONE BROADCAST -- every drone climbs to the SAME height first, regardless of its own configured offset -- and only the separate, later per-drone `goTo` sends each drone to its real distinct target. `formation_flight.py` was explicitly designed to avoid exactly this (`"stage 1: climb to individual heights first... so stage-2 XY convergence cannot bring two drones together"`); `simple_flight.py` has no equivalent protection. **Verdict: single-drone use is validated and safe. Multi-drone use is NOT safe as written** -- confirmed by data, not assumed. Do not use `simple_flight.py` for more than one drone without adding a staged takeoff first. |
| 2026-09-03 (6) | **Closed the last (4) caveat.** Bumped the 3-drone client timeout in `run_formation_flight_matrix.sh` (180s -> 280s, under the server's own 300s launch timeout; 2-drone kept at 180s, it never came close). Re-ran all six 3-drone cases: every one completed with a clean `rc=0`, zero timeouts. Numbers matched the original timeout-affected run almost exactly (e.g. `vertical/geo/3`: 12.4/42.7/30.5 mm mean error, both runs) -- confirms the earlier "pair by newest" fallback data was already correct, not just plausible. `formation_flight.py`'s sim validation now has no open caveats: 24/24 pair-checks PASS, cleanly, on both drone counts. |
| 2026-09-03 (5) | **Resolved the (4) tilt-transient finding — not a bug.** Isolated it first: reproduced with a single drone via `formation_flight.py` against `crazyflies_sim1.yaml` (rules out anything multi-drone: broadcast start, downwash coupling, formation offsets). Checked `flight.py` as a reference comparison and found it can't run in sim at all right now (`traj.mode` isn't a param the sim backend declares — separate, pre-existing gap, unrelated). Checked the trajectory CSV itself at its exact boundaries (`t=0`, `t=T`): velocity and acceleration are genuinely ~0, so not a rest-to-rest authoring bug (ruled out `export_poly4d.rs`'s known unfixed precision bug too — that only affects segments >=3.00s, and every segment in `figure8_mode1_kt0.05.csv` is under 0.73s). Scanning acceleration across the WHOLE trajectory (not just the boundary points) found the actual cause: `|acc|` peaks at 7.0 m/s^2 at t=0.45s (departing rest) and again at t=7.73s (0.45s before returning to rest) -- lining up exactly with every observed tilt spike. `atan(7.0/9.81) = 35.5 deg`, matching the observed 34-46 deg tilt almost exactly. **The controller is correctly executing a genuinely aggressive acceleration ramp the trajectory actually commands at its rest-to-rest boundaries** -- not a discontinuity, not a sim artifact, not specific to `formation_flight.py`. `run_formation.py`'s scenario curves apparently don't demand anything this aggressive at their own boundaries, which is why this was never noticed there. Nothing to fix. Worth knowing: `figure8 @ kt=0.05` is more aggressive at its ramp boundaries than "moderate" suggests -- pick a different trajectory/kt for a deliberately gentle validation flight. |
| 2026-09-03 (4) | **`formation_flight.py` fully sim-validated with quantitative verification, closing the gap from (2).** Built `verify_formation_flight_sim.py` (new — separate from `verify_formation_sim.py` because `formation_flight.py` commands a RIGID constant-offset formation, not the scenario library's time-varying curves) and `run_formation_flight_matrix.sh` (12 real flights: 3 formations x {2,3} drones x {geo,indi}). Extended `formation_flight.py` itself: writes a `t_start_sim` + commanded-geometry `meta.json` sidecar (same convention as `run_formation.py`), and switched `parse_args()` -> `parse_known_args()` so `--ros-args -p use_sim_time:=true` (required for the sidecar to mean anything) doesn't get rejected — a real, previously-latent bug, not cosmetic. **Result: 24/24 pair-checks PASS** (one row initially mislabeled FAIL under a too-strict max-based criterion I'd written into the new verifier; corrected to gate on MEAN error, matching `verify_formation_sim.py`'s established convention, before trusting any other result). Physics cross-check: vertical (downwash-coupled) cases show geo 12-43mm mean error vs INDI 0.7-2.2mm -- INDI cancelling the disturbance geometric can't, the thesis's core claim -- while horizontal/side_by_side (coplanar) cases are near-zero under BOTH controllers, matching the scenario library's own established "coplanar control case" signature exactly. Two caveats logged, not swept under the rug: (a) all six 3-drone cases hit the matrix's 180s client timeout (rc=124); the "pair by newest" fallback still produced physically-consistent data (cross-checked above). **Resolved same day, see (6): timeout bumped, re-run clean, numbers matched exactly.** (b) every one of the 12 cases shows an unusually large peak tilt (34-46 deg) landing exactly at the trajectory upload/start handoff, uniform across every formation/controller/drone-count including the near-zero-error coplanar cases -- confirmed real via raw quaternion inspection (unit-norm, correct timestamps). **Resolved same day, see (5): genuine commanded trajectory dynamics, not a bug.** Full table: `experiments/sim_validation/formation_flight_matrix_results.md`. |
| 2026-09-03 (3) | Wrote `flying_drone_stack/docs/dshot_rpm_source_plan.md`: a plan to re-test DShot bidirectional telemetry as an optional INDI RPM source (deck stays default). Grounded in the existing 2026-07-15/16/18 history, which is more nuanced than "DShot crashed, deck fixed it": the deck switch fixed the 07-15 crashes, but the *later* H1a diagnostic (`indi_gains.ff_free`) found the residual oscillation on the deck itself was ordinary gain amplification (present on the standard drone too, not brushless/RPM-source-specific), and DShot was never re-tried at the corrected, lower-oscillation gain point — a real, still-open question, not a settled one. Planning only, no firmware changes; explicitly not on the C.0-C.4 critical path. |
