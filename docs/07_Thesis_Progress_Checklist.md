...
| 4 | **Hybrid Neural-Augmented INDI** | Yes, alongside the INDI measurement | `controller=8` — **numerically verified, unflown** | ✅ built, ⬜ unflown. 2026-09-16: `ControllerTypeOot3`/`CONFIG_CONTROLLER_OOT3` slot-patched into `controller.h`/`controller.c`/`Kconfig`, `app-config`, and `bindings/cffirmware.i`. **Numerically verified same day**: 6/6 hand-picked test vectors match the reference's own compiled `controller_lee.c` (`use_nn=7`) to ~1e-9 on thrust and torque (`test_naindi_hybrid_reference.py`). One real test-harness bug found and fixed along the way (not a port bug — confirmed by comparing `nn_forward()` directly against the reference bit-for-bit first): the reference's `use_nn` path calls `usecTimestamp()` 3x per tick for unused profiling, inflating the scratch build's fake clock relative to this port's single call. **Still never flown — do not fly without clearing the hardware-validation gate.** `controller=7` is NOT this strategy — its `use_nn` flag is dead code, so it runs as Strategy 1's alt implementation, not NA-INDI |
