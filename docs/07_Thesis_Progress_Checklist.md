...
- [x] Multi-robot flight script `formation_flight.py` — **sim-validated 2026-09-03**: 24/24 pair-checks PASS across all 3 formations x {2,3} drones x {geo,indi}, quantitatively verified against `record_states` (not just crash-free). Physics cross-check matches the scenario library's own signature (INDI cancels vertical downwash, coplanar cases near-zero under both controllers). Two open items: 3-drone client timeout budget too tight (data still checked consistent), and a real ~35-46 deg tilt transient at every trajectory start/end handoff — see 2026-09-03 (4) History entry
