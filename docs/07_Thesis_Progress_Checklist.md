...
| **Next action — lab** | **2026-09-11 session done — see [`docs/lab_sessions/2026-09-11.md`](lab_sessions/2026-09-11.md) for the full account.** Stage 1 CLOSED clean (geo hover/circle/figure8 + restored-config INDI hover, all PASS — also caught and fixed a real trajectory-export ringing bug along the way, see history (21)). Stage 2: **2a/2b/2c done** (residual conditioning + filter no-op confirmed + `fc_bw` down made things worse, reverted). **2d (notch) CRASHED** — 88.7°/66.6° divergence within 0.7s of a stable hover, root cause NOT understood, `notch_en` must stay 0 until root-caused offline. **2e (shake-band, `kr`/`kw`) made real progress but no clean pass** across 4 attempts — peak attitude oscillation went 88°→7° through principled bisection + a coordinated `pos_gains` retune, but every attempt traded one axis (attitude peak vs position holding vs a small hover growing trend) for another. Ended the session reverted to the confirmed-clean locked point (`kr=2400`/`kw=170`, `pos_gains` 64/5/48/7). **Next session: (1) root-cause the notch crash offline before any further live notch test; (2) try the card's own H0 partition (`ctrl_mode=2` then `1`) — never actually run this session, and the natural next diagnostic given 4 straight gain attempts all converged on the same trajectory-tracking shake; (3) second brushless drone being prepped in parallel for the first 2-drone flight — gains are NOT frozen yet, so that should wait for a real stage-2e conclusion or a deliberate decision to proceed with the current locked config's known limitation.** |
