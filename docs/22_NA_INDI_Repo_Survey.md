...
modified. No submodules fetched.
...
**Missing — and this is the important gap:**
...
**Consequence: the onboard controller C code is NOT in what we received.** The `using_nn` /
`using_indi` flags are evaluated *offline in the notebook* to label pre-recorded flights; the
firmware that actually flew them lives in IMRCLab's `crazyflie-firmware` fork on branch
`upstream_lee`. `git submodule update --init` cannot recover it — the gitlinks were never
committed. To read their controller we would have to clone that fork separately.

`git submodule status` also errors (`no submodule mapping found ... for path
'uav_trajectories'`) — the repo's submodule config and tree disagree. Cosmetic, but it means
submodule commands fail outright.
...
1. **Their 2×2 is a template for our comparison table.** Same ablation structure, already
   validated in a published paper. Worth aligning our reporting to it where honest.
2. **`model_to_c_conversion.py`'s numerical C-vs-PyTorch check** — independent confirmation
...
3. **The spline-smoothing step on residual training data** is a concrete technique we do not
   currently use and could evaluate for C.2.
4. **`data_old/` is real flight data from a published method** — usable as a sanity target for
   our own analysis tooling, though it is single-drone-with-payload, not formation.
5. **Their no-payload results are the directly comparable baseline** to our single-drone case.

**What it is NOT useful for:** the onboard controller. It is not here. Any statement about
"how NA-INDI is implemented on the drone" cannot be sourced from this repo as received.

---

## 6. Open questions before this repo can be used further

1. **Do we fetch IMRCLab's `crazyflie-firmware` (branch `upstream_lee`) to read the actual
   controller?** ~1 GB and a fourth firmware tree to reason about. Not done — needs a
   decision.
2. **What exactly do the `using_nn` / `using_indi` flags map to on the drone** — parameters,
   compile-time switches, or separate builds? Unanswerable without item 1.
